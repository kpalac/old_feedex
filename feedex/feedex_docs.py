...
Usage: <b>feedex [parameters|filters] [actions] [arguments]</b>
    ...
    <b>Display parameters:</b>
        ...
        --notify-level=INT                      Specify notifications level
                                                0 - None, 1 - Summary, 2 - All, 3 - Flagged only
    ...
    <b>Entries:</b>
        ...
        --mark [ID] [N]                         Mark entry as read N times (increases weight in later ranking) and learn features
                                                options:
                                                --learn         Extract patterns from Entry
        ...
        --add-entry [TITLE] [TEXT]              Add an entry providing title and text. Useful for saving highlights or notes.
                                                ...
                                                --learn       Do you want to learn features from this entry for ranking news?
        ...
        --edit-entry [ID] [FIELD] [VALUE]       Edit ID'd Entry. Change [FIELD] to specified [VALUE]. See --help-entries for field names

        --add-entries-from-file [FILE]          
        --add-entries-from-pipe                 You can add Entries wholesale from a file or pipe input.
                                                For input file format see <b>--help-entries</b> option
    ...
    <b>Queries:</b>
        -q, --query [Phrase]                    Query entries with a search phrase:
                                                ...
                                                --learn          save search phrase in rules to add importance to incoming news 
                                                                 if they happen to contain it
                                                ...
                                                --last_week, --last_month, --last_quarter, --last_year   limit to 7, 31 or 365 days ago
                                                ...
                                                --handler=       limit to feed handler (rss, local, twitter - not yet implemented)
    ...
    <b>Handlers:</b>
        ...
        <b>local</b>    No internet protocol. Feeds are not fetched from the WWW, but can be populated by scripts (see --add-entries-from-file/pipe)
        <b>twitter</b>  <i>This handler is not yet implemented</i>
    ...
    <b>Rules (Keywords and Terms):</b>
        ...
        --edit-rule [ID] [FIELD] [VALUE]        Edit ID'd Rule. Change [FIELD] to specified [VALUE]. See --help-rules for field names
    ...
    <b>Data and Dev:</b>
        ...
        --export-rules [FILENAME]               Export rules to json file
        --import-rules [FILENAME]               Import rules from json file to current DB
        ...
        --reclassify [ID]                       Recalculate importance and flag stats for all/IDd entry
...
Below are field descriptions:
    ...
    <b>handler</b>                             protocol handler:
                                               <b>rss</b>, <b>twitter</b> (<i>not yet implemented</i>),<b>local</b> (no fetching, populated manually or by scripts)
...
Entries are stored in DB in 'entries' table. Below, are field descriptions:
    ...
    <b>guid</b>                Global identifier for entry provided by publisher (these, and links, are checked at parsing
                        to avoid duplicates)
    <b>comments, source, links</b>    
                        Data extracted from respective feed sections
    ...
    <b>user_marked</b>         Increased whenever flagging rule is matched. It causes entry to be highlighted on output.
...
For <b>--add-entry-from-file</b> and <b>--add-entry-from-pipe</b> nput can be given to mass-insert entries.
Input format is as follows (markers should start at the beginning of a line):

<b>->Entry:</b>         Marks beginning of a new entry
<b>->[FIELD NAME]:</b>  Marks eginning of a field value e.g. ->feed_id: 12

    Values can be int, float, NULL and string and compatibile with field's data types
...
->desc:
Example description
->pubdate_str: 2020-02-01 
->text:NULL
->Entry:
->feed_id:2
->title:Example title II
->desc:
Example description II
->pubdate_str: 2020-02-02 
->text:NULL
...
Rules are stored in DB in rules table. Below are field descriptions:
    ...
    <b>type</b>                matching type of a rule:
                        values:
                            0 - simple string mtching
                            1 - full text search (stemmed)
                            2 - full text search (exact)
                            3 - REGEX search
    ...
    <b>field_id</b>            ID of a field to be matched by a rule
                        values:
                            1-'lang',2-'author',3-'publisher',4-'contributors',5-'link',6-'title',7-'desc',8-'tags',9-'category',10-'text' 
    ...
    <b>learned</b>             is rule learned?
                        values
                            1-learned from entry, 2-learned from search phrase, 0-manual
