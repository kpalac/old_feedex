...
def convert_timestamp(datestring:str, **kargs):
    ...
    if isinstance(datestring, str):
        ...
        try:
            ...
            cli_msg( (-1, _('Timestamp convertion: {e}')) )
            return None
...
def validate_config(config, **kargs):
    ...
    if msg: 
        ...
        if not os.path.isfile(config.get('db_path')): return -1, _('Database file %a does not exist!'), config.get('db_path')
...
def clear_im_cache(xdays:int, db_hash:str, **kargs):
