...
def sanitize_arg(arg, tp, default, **kargs):
    ...
    if kargs.get('allow_none',False) and arg == None:
        return None
...
def load_json(infile, default):
    ...
    try:
        ...
        sys.stderr.write(f'Error reading from {infile}!')        
        return default
...
def save_json(ofile:str, data):
    ...
    try:
        ...
        sys.stderr.write(f'Error writing to {ofile}!')
        return -1
    return 0
...
def parse_efile(ifile:str, **kargs):
    ...
    if pipe:
        ...
        if not os.path.isfile(ifile): return -1
    ...
    if type(entry_list) not in (list, tuple): return -2
    ...
    for i,e in enumerate(entry_list):
        ...
        if type(e.get('feed_id')) != int or e.get('feed_id') < 0:
