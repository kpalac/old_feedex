...
def validate_config(config):
    ...
    for k,v in config.items():
        if k in CONFIG_INTS_NZ:
            if scast(v, int, None) is None or v <= 0:
        ...
        if k in CONFIG_BOOLS:
            if scast(v, bool, None) is None:
                sys.stderr.write(f'Invalid config option: {k}; Defaulting to {DEFAULT_CONFIG[k]}')
                new_config[k] = DEFAULT_CONFIG[k]
        if k in CONFIG_FLOATS:
            if scast(v, float, None) is None:
                sys.stderr.write(f'Invalid config option: {k}; Defaulting to {DEFAULT_CONFIG[k]}')
                new_config[k] = DEFAULT_CONFIG[k]
        if k in CONFIG_STRINGS:
            if scast(v, str, None) is None:
                sys.stderr.write(f'Invalid config option: {k}; Defaulting to {DEFAULT_CONFIG[k]}')
                new_config[k] = DEFAULT_CONFIG[k]
        if k in CONFIG_KEYS:
            if scast(v, str, None) is None or len(v) > 1:
                sys.stderr.write(f'Invalid config option: {k}; Defaulting to {DEFAULT_CONFIG[k]}')
                new_config[k] = DEFAULT_CONFIG[k]
...
def parse_efile(ifile:str, **kargs):
    ...
    if pipe:
        ...
        try:
            with open(ifile, "r") as f:
                contents = f.read()
        except OSError as e: sys.stderr.write(f'Error reading {efile}: {e}')

    entry_list = json.loads(contents)

    # Validate data received from json
    if not isinstance(entry_list, (list, tuple)): return -2

    for i,e in enumerate(entry_list):

        if type(e) is not dict: 
            sys.stderr.write(f'Element {i} is not a dictionary')            
            return -3

        if type(e.get('feed_id')) is not int or e.get('feed_id') < 0:
            sys.stderr.write(f'Element {i}: "feed_id" needs to be an int > 0')
            return -4

        if type(e.get('read')) is not int or e.get('read') < 0:
            sys.stderr.write(f'Element {i}: "read" needs to be an int >= 0')
            return -4
        if e.get('flag') not in (None, 0,1,2,3,4,5):
            sys.stderr.write(f'Element {i}: "flag" needs to be None or int in (0,1,2,3,4,5)')
            return -4
        
        if e.get('deleted') not in (None, 0,1):
            sys.stderr.write(f'Element {i}: "deleted" needs to be an int in (0,1)')
            return -4


        for k in ('charset','lang','title', 'desc', 'text', 'author', 'publisher', 'contributors', \
            'author_contact', 'publisher_contact', 'link', 'pubdate_str', 'guid', 'category'\
            'tags','comments','source','links','images','enclosures'):
            if type(e.get(k)) is not str and e.get(k) is not None:
                sys.stderr.write(f'Element {i}: "{k}" needs to be str or None')
                return -4
