...
class InfoDialog(Gtk.Dialog):
    ...
    def __init__(self, parent, title, text, **kargs):
        ...
        label = f_label(text, justify='center', selectable=False, wrap=True, markup=True)
        box1.pack_start(label, False,False, 3)

        if subt != None:
            sublabel = f_label(subt, justify='center', selectable=False, wrap=True, markup=True)
...
class YesNoDialog(Gtk.Dialog):
    ...
    def __init__(self, parent, title, text, **kargs):
        ...
        if subt != None: top_box.pack_start(sublabel, False, False, 0)
...
class NewFromURL(Gtk.Dialog):
    ...
    def __init__(self, parent, config, fields, **kargs):
        ...
        self.cat_combo = f_feed_combo(parent.FX, start_at=fields.get('category'), tooltip="Choose Category to assign this Channel to\nCategories are useful for quick filtering and organizing Channels")

        self.handler_combo = f_handler_combo(no_local=True, start_at=fields.get('handler'), connect=self.on_changed )
    ...
    def get_result(self):
        ...
        self.result = {'url' : self.url_entry.get_text(), 'parent_id' : category, 'handler': handler }
...
class EditCategory(Gtk.Dialog):
    ...
    def __init__(self, parent, category, **kargs):
        ...
        if kargs.get('new',True): title = "Add New Category"
        else: title = f"Edit {self.name} Category"
...
class ChangeCategory(Gtk.Dialog):
    ...
    def __init__(self, parent, feed_name, curr_category, **kargs):
        ...
        feed_label = f_label(f'Choose category for Channel <i><b>{GObject.markup_escape_text(feed_name)}</b></i>:', markup=True)
        self.cat_combo = f_feed_combo(parent.FX, tooltip="Choose category to assign this feed to", start_at=curr_category)
    ...
    def on_save(self, *args):
        self.response = 1
        self.result = f_get_combo(self.cat_combo)
...
Rules are also learned automatically when any Entry/Article is opened in Browser""")
    ...
    def get_result(self):
        self.result = { 'feed_id': f_get_combo(self.cat_combo),
    ...
    def on_restore(self, *args, **kargs):
        ...
        if new:
            ...
            f_set_combo(self.cat_combo, self.fields.get('feed_id'))
...
class EditRule(Gtk.Dialog):
    ...
    def __init__(self, parent, config, fields, **kargs):
        ...
        if kargs.get('new',True): title = 'Add new Rule'
...
It is used according to <b>Type</b> and should be compatibile with it (e.g. REGEX string)""")
        ...
        feed_label = f_label('Feed or Category:', wrap=False)
...
Sometimes, however, a rule can simply increase importance by its <b>weight</b> without flagging""") 
    ...
    def on_restore(self, *args):
        ...
        f_set_combo(self.type_combo, self.fields.get('type'))
        f_set_combo(self.field_combo, self.fields.get('field_id'))
        f_set_combo(self.feed_combo, self.fields.get('feed_id'))
        f_set_combo(self.lang_combo, self.fields.get('lang'), null_val=None)
        f_set_combo(self.case_combo, self.fields.get('case_insensitive'))

        self.weight_entry.set_text( scast( scast(self.fields.get('weight', None), float, self.config.get('default_rule_weight',2)), str, '2'  ))
        
        f_set_combo(self.flag_combo, self.fields.get('flag',-1))
    ...
    def validate_entries(self):
        ...
        if self.string_entry.get_text() == '':
            ...
            return False                    
        if f_get_combo(self.type_combo) == 3 and not check_if_regex(self.string_entry.get_text()):
...
class DisplayRules(Gtk.Dialog):
    ...
    def __init__(self, parent, header, store, **kargs):
        ...
        header_label = f_label(header, justify=kargs.get('justify'), wrap=True, markup=True)

        scwindow = Gtk.ScrolledWindow()

        self.rule_store = store
        self.rule_list = Gtk.TreeView(model=self.rule_store)
        self.rule_list.append_column( f_col('Name',0 , 1, resizable=True, clickable=True, start_width=300) )
        self.rule_list.append_column( f_col('REGEX String',0 , 2, resizable=True, clickable=True, start_width=300) )
        self.rule_list.append_column( f_col('Weight',0 , 3, resizable=True, clickable=True, start_width=200) )
        self.rule_list.append_column( f_col('Query Type',0 , 4, resizable=True, clickable=True, start_width=200) )
...
class DisplayWindow(Gtk.Dialog):
    ...
    def __init__(self, parent, title, text, **kargs):
        ...
        if kargs.get('close_on_unfocus',False):
            self.connect('focus-out-event', self.on_close)

        image = kargs.get('image')
        image_url = kargs.get('image_url')
        ...
        if image != None:
            ...
            self.label = f_label(text, justify=kargs.get('justify'), wrap=True, selectable=True, markup=kargs.get('markup',True))
        ...
        if kargs.get('save_as',False):
            save_as_button = f_button(None, 'document-save-symbolic', connect=self.on_save_as, tooltip='Save as...')
...
class AboutDialog(Gtk.Dialog):
    ...
    def __init__(self, parent, **kargs):
        ...
        main_label = f_label(None, justify='center', wrap=True, markup=True)
        secondary_label = f_label(None, justify='center', wrap=True, markup=True)
        author_label = f_label(None, justify='center', wrap=True, markup=True)
        website_label = f_label(None, justify='center', wrap=True, markup=True)
...
class CalendarDialog(Gtk.Dialog):
    ...
    def __init__(self, parent, **kargs):
        ...
        from_label = f_label('  From:', justify='left', selectable=False, wrap=False)
        to_label = f_label('    To:', justify='left', selectable=False, wrap=False)
...
class EditFeed(Gtk.Dialog):    
    """ Edit Feed dialog """
    def __init__(self, parent, fields, FX, **kargs):
        ...
        if kargs.get('new',False): title = 'Add new Feed'
        ...
        self.cat_combo = f_feed_combo(FX, tooltip="""Choose <b>Category</b> to assign this Feed to
Categories are useful for bulk-filtering and general organizing""")

        self.handler_combo = f_handler_combo(connect=self.on_changed)
...
Local feeds are updated only by scripts or CLI (<i>--add-entry</i>, <i>--add-entries-from-file</i>, or <i>--add-entries-from-pipe</i> options)""")
    ...
    def on_restore(self, *args):
        ...
        f_set_combo(self.cat_combo, self.fields.get('parent_id'))
        f_set_combo(self.handler_combo, self.fields.get('handler','rss'))
    ...
    def validate_entries(self, *args):
        ...
        if self.home_entry.get_text().strip() != '' and not check_url(self.home_entry.get_text()):
            ...
            return False
        if not check_url(self.url_entry.get_text()) and f_get_combo(self.handler_combo) in ('rss',):
    ...
    def get_result(self, *args):
        self.result['parent_id'] = f_get_combo(self.cat_combo)
        self.result['handler'] = f_get_combo(self.handler_combo)
...
""")
        ...
        #query_rule_weight_label = f_label('Default Query weight:')
    ...
    def validate_entries(self, *args):
        ...
        key = self.key_search_entry.get_text()
        if len(key) > 1 or (key not in FEEDEX_GUI_VALID_KEYS):
            self.err_label.set_markup('<span foreground="red">Invalid hotkey value for New Search Tab! Needs to be [a-zA-Z0-9]</span>')
            return False
        key = self.key_new_entry_entry.get_text()
        if len(key) > 1 or key not in FEEDEX_GUI_VALID_KEYS:
            self.err_label.set_markup('<span foreground="red">Invalid hotkey value for New Entry! Needs to be [a-zA-Z0-9]</span>')
            return False
        key = self.key_new_rule_entry.get_text()
        if len(key) > 1 or key not in FEEDEX_GUI_VALID_KEYS:
            self.err_label.set_markup('<span foreground="red">Invalid hotkey value for New Rule! Needs to be [a-zA-Z0-9]</span>')
            return False
