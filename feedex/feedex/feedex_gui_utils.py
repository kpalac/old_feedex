...
def f_user_agent_combo(**kargs):
    ...
    tooltip = kargs.get('tooltip',"""Sometimes it is needed to change user anger tag if the publisher does not allow download (e.g. 403 HTTP response)
...
def f_search_engine_combo(**kargs):
    ...
    tooltip = kargs.get('tooltip',"""Search engine to use for browser searches. Use <b>Q%</b> symbol to substitute for query""")
...
def f_feed_icon_combo(**kargs):
    ...
    tooltip = kargs.get('tooltip',"""Choose icon for this Channel. Leave empty to diplay downloaded logo, if present""")
...
def f_archive_time_combo(**kargs):
    ...
    store = [
    (now-(86400*31*3),'Older than 3 months'),
    (now-(86400*31*6),'Older than 6 months'),
    (now-(86400*31*12),'Older than 1 year'),
    (now-(86400*31*24),'Older than 2 years'),
    (now-(86400*31*36),'Older than 3 years')
    ]
...
def f_cli_color_combo(**kargs):
    ...
    tooltip = kargs.get('tooltip',"""Choose color for command line interface """)
    store = []
...
def f_feed_combo(FX, **kargs):
    ...
    if kargs.get('with_templates',False):
        for k,v in FEEDEX_REGEX_HTML_TEMPLATES.items():
            store.append( (-k, f"""{v.get('name',f'Template {k}')}""", 700) )
...
""")
    store = [('rss', 'RSS'), ('html', 'HTML'), ('script', 'Script')]
...
def f_field_combo(**kargs):
    """ Build field combo """
    all_label = kargs.get('all_label','-- All Fields --')
    store = []
...
def get_icons(feeds, ficons):
    ...
    for f,ic in ficons.items():
        ...
        except Exception as e: cli_msg((-7, 'Image error: %a', e))
...
def validate_gui_attrs(gui_attrs):
    ...
    for v in new_gui_attrs['flags'].values():
        if type(v) is not int: 
            new_gui_attrs['flags'] = {}
            cli_msg( (-1,'Flags column attributes invalid. Defaulting ...') ) 
            break
    ...
    for c in FX_DEF_COLS_FLAGS: 
        if c not in new_gui_attrs['flags_order']:
                ...
                cli_msg( (-1,'Flags column order invalid. Defaulting ...') )
                break
...
1: {'name':"""Template: NY Review of Books""",
...
from feedex_gui_dialogs_entts import *
from feedex_gui_dialogs_utils import *
