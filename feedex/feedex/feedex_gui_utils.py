...
def f_col(title:str, ctype:int,  model_col:int, **kargs):
    ...
    if ctype in (0,1):
        ...
        if ellipsize is not None: renderer.props.ellipsize = ELLIPSIZE_DICT.get(ellipsize)
...
def res(string:str):
    ...
    if url is None or url.startswith('http://feeds.feedburner.com'):
        return 0
...
def validate_gui_attrs(gui_attrs):
    ...
    for v in new_gui_attrs['feeds_expanded'].values():
        if type(v) is not bool: 
            ...
            sys.stderr.write('Expanded feeds invalid. Defaulting ...')
            break
    ...
    for v in new_gui_attrs['results'].values():
        if type(v) is not int: 
            ...
            sys.stderr.write('Result column attributes invalid. Defaulting ...')
            break
    ...
    for v in new_gui_attrs['contexts'].values():
        if type(v) is not int:
            ...
            sys.stderr.write('Contexts column attributes invalid. Defaulting ...')
            break
    ...
    for v in new_gui_attrs['terms'].values():
        if type(v) is not int: 
            new_gui_attrs['terms'] = {}
            sys.stderr.write('Terms column attributes invalid. Defaulting ...')
            break
    ...
    for v in new_gui_attrs['time_series'].values():
        if type(v) is not int: 
            ...
            sys.stderr.write('Time series column attributes invalid. Defaulting ...')
            break
    ...
    for v in new_gui_attrs['rules'].values():
        if type(v) is not int: 
            new_gui_attrs['rules'] = {}
            sys.stderr.write('Rules column attributes invalid. Defaulting ...')
            break
    ...
    for c in (25,4,3,5,7,8,9,10,12,14,15,16,17,18,19,6): 
        if c not in new_gui_attrs['results_order']:
                ...
                sys.stderr.write('Results column order invalid. Defaulting ...')
                break
    ...
    for c in (3,15,4,5,7,9,10,11,12,13,14,6):
        if c not in new_gui_attrs['contexts_order']:
                ...
                sys.stderr.write('Contexts column order invalid. Defaulting ...')
                break
    ...
    for c in (1,2,3): 
        if c not in new_gui_attrs['terms_order']:
                ...
                sys.stderr.write('Terms column order invalid. Defaulting ...')
                break
    ...
    for c in (1,2,3): 
        if c not in new_gui_attrs['time_series_order']:
                ...
                sys.stderr.write('Time series column order invalid. Defaulting ...')
                break
    ...
    for c in (1,2,3,4,5,6,7,8,9): 
        if c not in new_gui_attrs['rules_order']:
                ...
                sys.stderr.write('Rules column order invalid. Defaulting ...')
                break
