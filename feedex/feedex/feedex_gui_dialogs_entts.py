...
class EditCategory(Gtk.Dialog):
    ...
    def __init__(self, parent, category, **kargs):
        ...
        icon_label = f_label('Icon:')
        icon_combo, self.icon_entry = f_feed_icon_combo(tooltip='Choose icon for this Category')
        self.icon_button = f_button(None, 'folder-symbolic', connect=self._on_choose_icon, tooltip='Search local machine for icons')        
    ...
    def _choose_file(self, *args, **kargs):
        dialog = Gtk.FileChooserDialog(kargs.get('title',"Choose file"), parent=self, action=Gtk.FileChooserAction.OPEN)
    ...
    def _on_choose_icon(self, *args, **kargs):
        ...
        filename = self._choose_file(title='Choose Image', start_dir=start_dir)
...
class EditFlag(Gtk.Dialog):
    ...
    def __init__(self, parent, config, flag, **kargs):
        ...
        if self.new: title = 'Add new Flag'
        else: title = 'Edit Flag'
        ...
        name_label = f_label('Name:', wrap=False)
        self.name_entry = Gtk.Entry()
        self.name_entry.set_tooltip_markup('Display name for this Flag used in Queries etc.')
        desc_label = f_label('Description:', wrap=False)
        self.desc_entry = Gtk.Entry()

        color_label = f_label('Color:')
        ...
        color_cli_label = f_label('Color (CLI):')
        self.color_cli_combo = f_cli_color_combo()

        id_label = f_label('ID:', wrap=False)
        self.id_entry = Gtk.Entry()
        self.id_entry.set_tooltip_markup("""Sometimes it is useful to set up ID manually e.g. to add description to flag deleted before""")

        self.err_label = f_label('', wrap=False)

        self.save_button = f_button('Save','object-select-symbolic', connect=self.on_save)
        self.cancel_button = f_button('Cancel','action-unavailable-symbolic', connect=self.on_cancel)
        self.restore_button = f_button('Restore','edit-redo-rtl-symbolic', connect=self.on_restore)
...
Weights from leaned rules as well as the ones from manually added ones sum up and position an Article""")
        ...
        self.flag_combo = f_flag_combo(self.FX, filters=False, tooltip="""Main reason for manually added rules is to flag interesting incomming articles independently of importance ranking
...
class EditFeedRegex(Gtk.Dialog):
    ...
    def __init__(self, parent, FX, regexes, ifeed, **kargs):
        ...
        Gtk.Dialog.__init__(self, title="REGEX strings for HTML parsing", transient_for=parent, flags=0)
        self.set_default_size(kargs.get('width',1000), kargs.get('height',500))
        box = self.get_content_area()

        entries_label = f_label('<b>Entries:</b>', markup=True)
        self.entries_entry = Gtk.Entry()
        self.entries_entry.set_tooltip_markup("""This REGEX should extract list of strings for Entries to be parsed by entry-specific REGEXes
Parsing will be done match by match with each REGEX below
""")
        
        title_label = f_label('Title:')
        self.title_entry = Gtk.Entry()
        link_label = f_label('Link:')
        self.link_entry = Gtk.Entry()
        desc_label = f_label('Description:')
        self.desc_entry = Gtk.Entry()
        author_label = f_label('Author:')
        self.author_entry = Gtk.Entry()
        category_label = f_label('Category:')
        self.category_entry = Gtk.Entry()
        text_label = f_label('Additional Text:')
        self.text_entry = Gtk.Entry()
        images_label = f_label('Images:')
        self.images_entry = Gtk.Entry()
        pubdate_label = f_label('Published Date:')
        self.pubdate_entry = Gtk.Entry()

        title_feed_label = f_label("Feed's Title:")
        self.title_feed_entry = Gtk.Entry()
        pubdate_feed_label = f_label("Feed's: Published Date:")
        self.pubdate_feed_entry = Gtk.Entry()
        image_feed_label = f_label("Feed's Image:")
        self.image_feed_entry = Gtk.Entry()
        charset_feed_label = f_label("Feed's Encoding:")
        self.charset_feed_entry = Gtk.Entry()
        lang_feed_label = f_label("Feed's Language:")
        ...
        self.prev_notebook.append_page(self.prev_box_html, f_label("HTML"))
        self.prev_notebook.append_page(self.prev_box_parsed, f_label("Parsing Preview"))

        self.load_from_button = f_button('Load from...','object-select-symbolic', connect=self.on_load_from, tooltip="Load REGEXes from other Chanel")
        self.feed_combo = f_feed_combo(self.FX, no_empty=True, with_categories=False, with_feeds=True, with_templates=True)

        self.err_label = f_label('', markup=True)

        self.save_button = f_button('Accept','object-select-symbolic', connect=self.on_save, tooltip="Save Entry")
        self.cancel_button = f_button('Cancel','action-unavailable-symbolic', connect=self.on_cancel)
        ...
        self.test_button = f_button('Test REGEXes','system-run-symbolic', connect=self.on_test_regex, tooltip="")
    ...
    def on_test_regex(self, *args, **kargs):
        if self.ifeed.get('url') in ('',None):
            self.err_label.set_markup(gui_msg( (-1, 'Link URL is empty. Cannot download resource') ))
            return -1
        ...
        if self.handler.error:
            self.err_label.set_markup(gui_msg( (-1, f'Handler error: {self.handler.error_str}') ))
            return -1

        demo_string = f"""
        
        
Feed title: <b>{esc_mu(feed_title)}</b>
Feed Published Date: <b>{esc_mu(feed_pubdate)}</b>
Feed Image Link: <b>{esc_mu(feed_img)}</b>
Feed Character Encoding: <b>{esc_mu(feed_charset)}</b>
Feed Language Code: <b>{esc_mu(feed_lang)}</b>
------------------------------------------------------------------------------------------------------------------------------
Extracted entries ({len(entries)}):
"""
        for e in entries:

            demo_string = f"""{demo_string}
    ------------------------------------------------------------------
    Title: <b>{esc_mu(e.get('title'))}</b>
    Link: <b>{esc_mu(e.get('link'))}</b>
    GUID: <b>{esc_mu(e.get('guid'))}</b>
    Published: <b>{esc_mu(e.get('updated'))}</b>
    Description: <b>{esc_mu(e.get('description'))}</b>
    Category: <b>{esc_mu(e.get('category'))}</b>
    Author: <b>{esc_mu(e.get('author'))}</b>
    Additional Text: <b>{esc_mu(e.get('content',{}).get('value',''))}</b>
    Image HREFs: <b>{esc_mu(e.get('images'))}</b>"""

        demo_string = f"""{demo_string}
------------------------------------------------------------------------------------------------------------------------------
<i><b>Entry samples:</b></i>
{esc_mu(entry_sample)}
"""
    ...
    def on_load_from(self, *args, **kargs):
        ...
        if feed_id < 0:
            feed_dict = FEEDEX_REGEX_HTML_TEMPLATES.copy()
            feed = feed_dict.get(-feed_id)
            del feed['name']
    ...
    def validate_entries(self, *args):
        self.ifeed.merge(self.result)
        err = self.ifeed.validate_regexes()
        if err != 0:
            self.err_label.set_markup(gui_msg(err))
            return False
        return True
...
<i>Updating on every fetch can cause unnecessary overhead</i>""")
        ...
        self.fetch_button = Gtk.CheckButton.new_with_label('Fetch News?')
        self.fetch_button.set_tooltip_markup("""If disabled, Channel's news will not be fetched unless manually requested""")
        ...
        script_file_label = f_label('Fetching script:')
        self.script_entry = Gtk.Entry()
        self.script_entry.set_tooltip_markup("""Script/command to be executed to fetch entries for this Channel. 
This script should return a <b>JSON string</b>
See <b><i>feedex --help-scripting</i></b> for detailed specification""")
        self.script_button = f_button(None, 'system-run-symbolic', connect=self._on_choose_script, tooltip='Search local machine for script file')

        icon_label = f_label('Icon:')
        icon_combo, self.icon_entry = f_feed_icon_combo()
        self.icon_button = f_button(None, 'folder-symbolic', connect=self._on_choose_icon, tooltip='Search local machine for icons')

        self.regex_button = f_button('REGEX',None, connect=self._on_define_regex, tooltip="Define <b>REGEX strings</b> for <b>HTML</b> parsing")
    ...
    def _on_define_regex(self, *args, **kargs):
        ...
        dialog = EditFeedRegex(self, self.parent.FX, self.regexes, self.get_data(for_regex=True))
        dialog.run()
    ...
    def _on_choose_script(self, *args, **kargs):
        ...
        filename = self._choose_file(title='Choose Script File', start_dir=start_dir)
    ...
    def on_changed(self, *args):
        ...
        if handler == 'rss':
            ...
            self.url_entry.set_tooltip_markup("""Valid <b>URL</b> to WWW page to be parsed. 
For HTML parsing it is good to use Webpages with as little JavaScript as possible""")
        elif handler in ('local', 'script'):
            self.url_entry.set_tooltip_markup("""For <b>local</b> and <b>script</b> feeds this can be any string, as it is not used
...
Scripted feeds are updated by script defined below during fetching""")
        ...
        if handler == 'html': self.regex_button.set_sensitive(True)
