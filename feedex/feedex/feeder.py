...
#
    ...
    def locked(self, **kargs):
        ...
        while tm <= timeout or self.wait_indef:
            ...
            if lock is not None or self.db_error is not None: sys.stderr.write(f"Database locked... Waiting ... {tm}")     
            else:
                self.lock()
                return False
            tm = tm + 1
    ...
    def qr_sql(self, sql:str,*args, **kargs):
        ...
        if kargs.get('ignore_errors', True):
            self.db_error = None
...
"""
    ...
    def g_db_maintenance(self, **kargs):
        ...
        if self.db_error is None:
            ...
            yield 0, 'DB maintenance failed! (%a)', self.db_error
