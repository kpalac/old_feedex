...
#
                        ...
                        if os.path.isfile(scr_path):
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    yield -1, f'Error restoring %a database! {e}', db_path
                                finally: 
                                    self.log(False, f'Database {db_path} resotred ...')
                                    yield 0, f'Database %a restored ...', db_path

                                self.db_status = 'Version update error'
                                return -1
            
            yield 0, 'Database updated successfully'
            yield 0, 'Deleting backup ...'
            try: os.remove(f'{db_path }.bak')
            except OSError as e:
                self.db_status = f'Error removing {db_path}.bak: {e}'
                yield -1, f'Error removing {db_path}.bak: %a', e
                return -1 
            yield 0, 'Backup database removed successfully'
    ...
    def locked(self, **kargs):
        ...
        self.MC.ret_status = cli_msg( (-4, f'Timeout reached ({self.conn_id}). Aborting...') )
        return True
        ...
        for feed in self.MC.feeds:
            ...
            if not update_only:
                ...
                for item in handler.fetch(self.feed, force=force, pguids=pguids, plinks=plinks, last_read=last_read, last_checked=last_checked):
                    ...
                    if isinstance(item, EntryContainer):
                        ...
                        if err != 0: 
                            yield -7, f'Error while processing entry {self.new_items}: Invalid data type for %a', err
                            continue
            ...
            if (scast(self.feed['autoupdate'], int, 0) == 1 or update_only) and not handler.no_updates:
                ...
                if msg != 0: yield -3, 'Handler error: %a', msg
                    ...
                    if err != 0: yield -7, 'Invalid data type for %a', err
                    else:
...
Statistics for database: {mb}{self.db_path}{me}
...
"""
    ...
    def g_db_maintenance(self, **kargs):
        ...
        self.MC.db_lock = False
        yield 0, 'Starting DB miantenance'
    ...
    def r_port_data(self, ex:bool, pfile:str, mode:str, **kargs):
        ...
        if ex:
            ...
            if ldata == (): return -6, 'Error reading data from %a', pfile
            ...
            if mode == 'feeds':
                ...
                if err != 0: return -2, 'DB error: %a', err
                    ...
                    return 0, 'Flags successfully imported from %a', pfile
    ...
    def g_archive(self, time, target_db, **kargs):
        ...
        if os.path.isfile(target_db) or os.path.isdir(target_db): 
            yield -6, 'File already exists! Aborting...'
            return -6
        
        db_backup = f'{self.db_path}.bak'
        yield 0, "Making safety backup to %a", db_backup
        self.conn.rollback()
        self.conn.close()
        try: copyfile(self.db_path, db_backup)
        except OSError as e:
            self.db_status = f'Error creating database backup to {db_path}.bak: {e}'
            yield -2, 'Error creating database backup to {db_path}.bak: %a', e
            return -1
        finally:
            self._connect_db()
        ...
        if target_db_conn.db_error is not None:
            yield -2, 'Target DB error: %a', target_db_conn.db_error            
            return -2
        else: yield 0, 'Database %a created ...', target_db
        
        if type(time) is int: timestamp = time
        else: timestamp = convert_timestamp(time)
        if timestamp is None: 
            yield -7, 'Error converting time: %a', time
            return -7


        yield 0, 'Counting entries to be archived ...'
        entry_count = self.qr_sql("""select count(e.id) from entries e where e.pubdate <= :timestamp""",{'timestamp':timestamp}, one=True)
        entry_count = slist(entry_count, 0, 0)
        if entry_count == 0: 
            yield 0, 'Nothing to be archived ... Aborting ...'
            return 0


        if not with_rules:

            yield 0, 'Calculating archive weights for rules\' contexts...'

            err = self.run_sql_lock("""update rules
...
where ee.pubdate <= :timestamp and rr.learned = 1)""", {'timestamp':timestamp})
            if err != 0: 
                yield -2, 'DB Error: %a', err
                return -2
            ...
            yield 0, 'Transferring rules to archived DB ...'
            rules = self.qr_sql("""SELECT rr.* from rules rr
join entries ee on ee.id = rr.context_id
where ee.pubdate <= :timestamp and rr.learned = 1""", {'timestamp':timestamp}, all=True)

            if len(rules) == 0: yield 0, 'No rules to archive ...'
            else:
                err = target_db_conn.run_sql_lock(self.rule.insert_sql(all=True), rules, many=True)
                if err != 0: 
                    yield -2, 'DB Error while transferring rules: %a', err
                    return -2
            
        
        yield 0, 'Transferring entries...'

        for offset in range(0, entry_count, 500):
            entries = self.qr_sql("""select * from entries where pubdate <= :timestamp LIMIT 500 OFFSET :offset""", {'timestamp':timestamp, "offset":offset}, all=True)
            yield 0, f'Moving {offset}/{entry_count}'
            err = target_db_conn.run_sql_lock(self.entry.insert_sql(all=True), entries, many=True)
            if err != 0: 
                yield -2, 'DB Error while transferring entries: %a', err
                return -2

        yield 0, 'Transferring feed data ...'
        feeds = self.qr_sql(GET_FEEDS_SQL, all=True)
        err = target_db_conn.run_sql_lock(self.feed.insert_sql(all=True), feeds, many=True)
        if err != 0: 
            yield -2, 'DB Error while transferring feeds: %a', err
            return -2

        yield 0, 'Transferring flags ...'
        ...
        if err != 0: 
            yield -2, 'DB Error while transferring flags: %a', err
            return -2

        yield 0, 'Transferring manual rules ...'
        rules = self.qr_sql("""select * from rules where coalesce(learned,0) = 0""", all=True)
        err = target_db_conn.run_sql_lock(self.rule.insert_sql(all=True), rules, many=True)
        if err != 0: 
            yield -2, 'DB Error while transferring manual rules: %a', err
            return -2


        if with_rules:
            yield 0, 'Removing rules from current DB...'
            err = self.run_sql_lock("""delete from rules where id in
(SELECT rr.id from rules rr
join entries ee on ee.id = rr.context_id
where ee.pubdate <= :timestamp and rr.learned = 1)""", {'timestamp':timestamp})
            if err != 0: 
        ...
        yield 0, 'Removing old entries from current DB...'
        err = self.run_sql_lock("""delete from entries where pubdate <= :timestamp""", {'timestamp':timestamp})
        if err != 0:
            yield -2, 'DB Error: %a', err
            return -2

        err = False
        for m in self.g_db_maintenance(**kargs): 
            yield m
            if m[0] < 0: err = True
        ...
        try: os.remove(db_backup)
            ...
            self.db_status = f'Error removing {db_backup}: {e}'
            yield -1, f'Error removing {db_backup}: %a', e
            return -1 
        yield 0, 'Backup database removed successfully'

        if not err:
            yield 0, 'DB successfully archived to %a', target_db
            return 0
