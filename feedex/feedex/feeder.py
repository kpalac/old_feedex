...
# FEEDS
    ...
    def del_feed(self, id:int, **kargs):
        """Delete feed data - by ID only"""
        for _ in self.g_del_feed(id, **kargs): pass
    def g_del_feed(self, id:int, **kargs):
    ...
    def add_feed(self, name:str, fields:dict, **kargs): 
        """Wrapper for adding a feed to database"""
        for _ in self.g_add_feed(name, fields, **kargs): pass
    def g_add_feed(self, name:str, fields:dict, **kargs):
        """Wrapper for adding a feed to database (generator method)"""
        is_category = fields.get('is_category',False)
        if (not is_category) and (not check_url(kargs.get('url', None)) and kargs.get('handler','rss') in ('rss',) ):
            if self.gui: yield -4, kargs.get('url','<<UNKNOWN>>')
            self._msg(-4, kargs.get('url','<<UNKNOWN>>'))
            return -1
        if (not is_category) and (not check_url(kargs.get('link', None))):
            if self.gui: yield -4, kargs.get('link','<<UNKNOWN>>')
            self._msg(-4, kargs.get('link','<<UNKNOWN>>'))
            return -1
        ...
        if not is_category:
            self.feed['handler'] = kargs.get('handler','rss')
        else:
        ...
        if is_category:
            ...
            if self.gui: yield 3, kargs.get('url', '<<UNKNOWN>>')
            self._msg(3, kargs.get('url', '<<UNKNOWN>>'))
    ...
    def add_feed_from_url(self, url, **kargs): 
        ...
        for _ in self.g_add_feed_from_url(url, **kargs): pass
    def g_add_feed_from_url(self, url, **kargs):
        ...
        parent_id = self.resolve_category(kargs.get('parent_id'))
...
values(:url, :login, :domain, :passwd, :auth, :interval, 0, :handler, :parent_id, 1)""",
    ...
    def edit_feed(self, id, fields:dict, **kargs):
        """Edit Feed's parameter - wrapper for update"""
        for _ in self.g_edit_feed(id, fields, **kargs): pass
    def g_edit_feed(self, id, fields:dict, **kargs):
...
# ENTRIES
    ...
    def edit_entry(self, id, fields:dict, **kargs):
        ...
        for _ in self.g_edit_entry(id, fields, **kargs): pass
    ...
    def del_entry(self, id:int, **kargs):
        """Delete entry by id """
        for _ in self.g_del_entry(id, **kargs): pass
    def g_del_entry(self, id:int, **kargs):
    ...
    def run_entry(self, **kargs):
        ...
        for _ in self.g_run_entry(**kargs): pass
    ...
    def add_entries(self, **kargs):
        for _ in self.g_add_entries(**kargs): pass
    def g_add_entries(self, **kargs):
        ...
        if elist == -1:
            ...
            return -1
        if elist == -2:
            if self.gui: yield -49
            self._msg(-49)
            return -1
        if elist == -3:
            if self.gui: yield -50
            self._msg(-50)
            return -1
        ...
        for e in elist:
            ...
            if learn and self.entry['tokens'] not in (None,'') and coalesce(self.entry['read'],0) > 0:
                if self.gui: yield 15
                self._msg(15, None)
                self.learn_rules(self.LP.features, context_id=context_id, lang=self.entry['lang'], ignore_lock=True)
...
#
    ...
    def learn_rules(self, features:dict, **kargs):
        for _ in self.g_learn_rules(features, **kargs): pass
    def g_learn_rules(self, features:dict, **kargs):
        ...
        if context_id != None and context_id > 0:
            self._run_sql_lock("delete from rules where context_id = ? and learned = 1", (context_id,) )
    ...
    def add_rule(self, fields:dict, **kargs):
        for _ in self.g_add_rule(fields, **kargs): pass
    def g_add_rule(self, fields:dict, **kargs):
    ...
    def del_rule(self,id:int, **kargs):
        for _ in self.g_del_rule(id, **kargs): pass
    def g_del_rule(self,id:int, **kargs):
        """ Delete rule by ID """
    ...
    def edit_rule(self, id:int, fields:dict, **kargs):
        for _ in self.g_edit_rule(id, fields, **kargs): pass
    def g_edit_rule(self, id:int, fields:dict, **kargs):
...
#       Fetching
    ...
    def fetch(self, **kargs):
        for _ in self.g_fetch(**kargs): pass
...
# Utilities 
    ...
    def clear_history(self, **kargs):
        for _ in self.g_clear_history(**kargs): pass
    def g_clear_history(self, **kargs):
    ...
    def delete_query_rules(self, **kargs):
        for _ in self.g_delete_query_rules(**kargs): pass
    def g_delete_query_rules(self, **kargs): 
    ...
    def delete_learned_rules(self, **kargs):
        for _ in self.g_delete_learned_rules(**kargs): pass
    def g_delete_learned_rules(self, **kargs):
    ...
    def empty_trash(self, **kargs):
        for _ in self.g_empty_trash(**kargs): pass
    def g_empty_trash(self, **kargs):
    ...
    def recalculate(self, **kargs):
        ...
        if entry_id == 0:
            ...
            if verbose:
                self._log(False, "Recalculating for entry:", str(kargs.get('id')))
...
"""
    ...
    def db_maintenance(self, **kargs):
        for _ in self.g_db_maintenance(**kargs): pass        
    def g_db_maintenance(self, **kargs):
        ...
        doc_count = self.get_doc_count(igonre_lock=True)
    ...
    def port_data(self, ex:bool, pfile:str, mode:str, **kargs):
        ...
        if ex:
            ...
            if ldata == (): return -1
            if type(ldata) not in (list, tuple):
                sys.stderr.write(f'Invalid data format!')
                return -1
