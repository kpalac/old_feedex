...
#
    ...
    def log(self, err:bool, *args, **kargs):
        ...
        try:
            ...
            cli_msg( (-1,f"Could not open log file {self.config.get('log','<<<EMPTY>>>')}: %a", f'{e}') ) 
    ...
    def _g_connect_db(self):
        ...
        # This is for queries and needs to be done in SQLite
        try:
            self.curs.execute("PRAGMA case_sensitive_like=true;")
            self.conn.commit()
        except sqlite3.Error as e:
            yield -2, 'Error setting up PRAGMA: %a', e
            return -2
    ...
    def _reset_connection(self, **kargs):
        ...
        try:
            ...
            if kargs.get('log',False): self.log(True, "Connection reset failed")
    ...
    def lock(self, **kargs):
        ...
        try:
            ...
            self.log(True, f'DB error (locking): {err}')
            cli_msg( (-2, 'DB error (locking): %a', err) )
            return e
    ...
    def unlock(self, **kargs):
        ...
        while tm <= self.timeout or self.wait_indef:        
            try:
                ...
                self.db_error = err
                self.MC.ret_status = -2
                self.log(True, f'DB error (unlocking): {err}')
                cli_msg( (-2, 'DB error (unlocking): %a', err) )
            tm = tm + 1
            time.sleep(1)
        
        cli_msg((-1, 'Failed to unlock DB'))
    ...
    def locked(self, **kargs):
        ...
        while tm <= timeout or self.wait_indef:
            lock = self.qr_sql("select * from params where name = 'lock'", one=True)
            if lock is not None or self.db_error is not None: cli_msg( (-4, f"Database locked... Waiting ... {tm}") )     
            else:
                self.lock()
                return False
            tm = tm + 1
        ...
        self.MC.ret_status = cli_msg( (-4, 'Timeout reached. Aborting...') )
        return True
    ...
    def _run_sql(self, query:str, vals:list, **kargs):
        ...
        try:
            ...
            self.db_error = err
            self.MC.ret_status = -2
            self.log(True, f'DB error (write): {err} ({query})')
            cli_msg( (-2, f'DB error (write): %a ({query})', err) )
    ...
    def qr_sql(self, sql:str,*args, **kargs):
        ...
        try:
            if many: return self.curs.executemany(sql, *args)
            else:
                if fetch_all: return self.curs.execute(sql, *args).fetchall()
                elif fetch_one: return self.curs.execute(sql, *args).fetchone()
            ...
            self.db_error = err
            self.MC.ret_status = -2
            self.log(True, f'DB error (read): {err} ({sql})')
            cli_msg( (-2, f'DB error (read): %a ({sql})', err) )
            return ()
...
#       Fetching
    ...
    def get_last(self):
        ...
        last = self.qr_sql("select max(val) from params where name = 'last'", one=True)
        if self.db_error is not None: return self.MC.last_fetch
...
and coalesce(ff.deleted,0) <> 1
    ...
    def db_stats(self, **kargs):
        ...
        last_update = self.qr_sql("select max(val) from params where name = 'last'", one=True, ignore_errors=False)
        first_update = self.qr_sql("select min(val) from params where name = 'last'", one=True, ignore_errors=False)
...
"""
    ...
    def check_due_maintenance(self):
        last_maint = slist(self.qr_sql("""select coalesce(val,0) from params where name = 'last_maintenance' """, one=True), 0, 0)
    ...
    def g_db_maintenance(self, **kargs):
        ...
        doc_count = slist( self.qr_sql('select count(e.id) from entries e', one=True, ignore_errors=False), 0, 0)
        doc_count = scast(doc_count, int, 0)
        avg_weight = slist( self.qr_sql('select avg(coalesce(weight,0)) from entries', one=True, ignore_errors=False), 0, 0)
        ...
        self.qr_sql("insert into params values('avg_weight', :avg_weight)", {'avg_weight':avg_weight}, one=True, ignore_errors=False)

        self.qr_sql("""delete from params where name = 'last_maintenance'""", one=True, ignore_errors=False)
        self.qr_sql("""insert into params values('last_maintenance',:doc_count)""", {'doc_count':doc_count}, one=True, ignore_errors=False)
