...
class Feeder:
    ...
    def __init__(self, top_parent, **kargs):
        ...
        if self.main_thread:
            ...
            if self.debug: print('Main connection established...')
...
#
    ...
    def _g_connect_db(self):
        ...
        if self.debug: print(f"Connected to {db_path} ({self.conn_id})")
    ...
    def qr_sql(self, sql:str,*args, **kargs):
        ...
        while tm <= LOCAL_DB_TIMEOUT: # This will queue the query if there is something else going on locally...
            if not self.MC.db_lock: 
                ready = True
                break
            elif self.debug: cli_msg ( (-2, f'DB locked locally ({self.conn_id})... waiting {tm}') )
            tm += 1
            time.sleep(1)
    ...
    def _disconnect(self):
        ...
        if self.debug: print(f'Disconnecting connection {self.conn_id}')
        self.MC.lock.acquire()
        self.MC.conns -= 1
        self.MC.lock.release()
