    ...
    def on_date_changed(self, *args):
        ...
        if f_get_combo(self.qtime_combo) == 'choose_dates':
            ...
            if dialog.response == 1:
                ...
                new_d = (f'_{dialog.result["date_string"]}', dialog.result['date_string'])
                model.append(new_d)
...
""")
    ...
    def _create_list(self, feed_sum, feed_filter, ignore_filters, **kargs):
        ...
        if self.type in ('places','search','similar','trash','feed'):
            ...
            if kargs.get('add_top_entry', False):
                self.tmp_store.append(self._result_store_item(self.top_entry) )
            ...
            if kargs.get('add_top_entry', False): 
                self.results.append(self.top_entry.listify())
    ...
    def apply_changes(self, id:int, action, **kargs):
        ...
        if action in ('open', 'read','unread'):
            for i,r in enumerate(self.result_store):
                if r[1] == id:
                    if action == 'unread':
                        ...
                        if weight_c is not None: self.result_store[i][weight_c] = 400
                    else:
                        self.result_store[i][read_c] += 1
                        if weight_c is not None: self.result_store[i][weight_c] = 700
            ...
            for i,r in enumerate(self.result_store):
                if r[1] == id:
                    if action == 'unflag':
                        ...
                        if weight_c is not None: self.result_store[i][col_c] = None
                    else:
                        self.result_store[i][flag_c] = entry
                        if weight_c is not None: self.result_store[i][col_c] = self._get_flag_color(entry)
            ...
            if entry is None: entry = list(self.parent.FX.sqlite_cur.execute(f"select {RESULTS_COLUMNS_SQL}\nwhere e.id = ?",(id,)).fetchone())
