...
class FeedexTab(Gtk.ScrolledWindow):
    ...
    def __init__(self, parent, **kargs):
        ...
        if self.type in ('places','search','similar','trash','feed'):
            ...
            self.column_types   = (GdkPixbuf.Pixbuf, int,int, str,str,str,str,str, int,int,int, float,float,float,int,str, str,  int)
        elif self.type == 'terms':
...
""")
    ...
    def _create_list(self, feed_sum, feed_filter, ignore_filters, **kargs):
        ...
        self.parent.entry_changes_q.append((self.uid, 'finished_search'))
    ...
    def create_rules_list(self):
        """ Reload rules store """
        self.tmp_store = Gtk.ListStore(*self.column_types)
        self.results = self.parent.FX.curs.execute("""select * from rules where learned <> 1 order by id desc""").fetchall()
    ...
    def apply_changes(self, entry, action, **kargs):
        ...
        """
        if self.type in ('rules', 'terms', 'time_series'): return -1
        ...
        if action == 'edit':
                    ...
                    if self.type in ('places','search','similar','feed'):
                        for ixx, rs in enumerate(self.result_store):
                            if rs[1] == id:
                                ...
                                self.result_store.insert(ixx, self._result_store_item(self.result, ix))
                                break
                        break
            ...
            entry = list(self.parent.FX.curs.execute(f"select {RESULTS_COLUMNS_SQL}\nwhere e.id = ?",(entry['id'],)).fetchone())
