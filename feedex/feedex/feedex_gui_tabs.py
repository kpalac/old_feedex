...
class FeedexTab(Gtk.ScrolledWindow):
    ...
    def __init__(self, parent, **kargs):
        ...
        self.type = kargs.get('type','search')
        ...
        # Search bar
        if self.type in ('search','contexts','terms','time_series'):
            ...
            if self.type in ('search','contexts','time_series'):
                ...
                self.qtime_combo        = f_time_combo(connect=self.on_date_changed, ellipsize=False, tooltip="""Filter by date\n<i>Searching whole database can be time consuming for large datasets</i>""")
        ...
        if self.type in ('places','search','similar','trash','feed'):
            ...
            self.result_store   = Gtk.ListStore(GdkPixbuf.Pixbuf, int,int, str,str,str,str,str, int,int,int, float,float,float,int,str, str)

        elif self.type == 'terms':
            self.result_store   = Gtk.ListStore(str, float, int)

        elif self.type == 'time_series':
            self.result_store   = Gtk.ListStore(str, str, int)


        elif self.type == 'rules':
            self.result_store   = Gtk.ListStore(int, str, str, float, str, str, str, str, str, str, str)
            

        self.result_list = Gtk.TreeView(model=self.result_store)        


        if self.type in ('places','search','similar','feed'):
            ...
            for c in self.parent.gui_attrs.get('results_order', (25,4,3,5,7,8,9,10,12,14,15,16,17,18,19,6)): self.result_list.append_column(columns[c])
...
Right-click for more options""")
            ...
            self.result_list.append_column( f_col('', 3, 0, resizable=False, clickable=False, width=16, reorderable=False) )
            columns = {}
            columns[3] = f_col('Context', 1, 3, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('3',650), name='3')
            columns[15] = f_col('Date', 0, 15, color_col=16, sort_col=8, start_width=self.parent.gui_attrs.get('contexts',{}).get('15',150), name='15')
            columns[4] = f_col('Source', 0, 4, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('4',200), name='4')
            columns[5] = f_col('Title', 0, 5, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('5',650), name='5')
            columns[7] = f_col('Publ. Timestamp', 0, 7, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('7',200), name='7')
            columns[9] = f_col('Read?', 0, 9, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('9',200), name='9')
            columns[10] = f_col('Flagged?', 0, 10, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('10',200), name='10')
            columns[11] = f_col('Importance', 0, 11, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('11',200), name='11')
            columns[12] = f_col('Weight', 0, 12, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('12',200), name='12')
            columns[13] = f_col('Readability', 0, 13, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('13',200), name='13')
            columns[14] = f_col('Word Count', 0, 14, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('14',200), name='14')
            columns[6] = f_col('Link', 0, 6, color_col=16, start_width=self.parent.gui_attrs.get('contexts',{}).get('6',400), name='6')
            for c in self.parent.gui_attrs.get('contexts_order', (3,15,4,5,7,9,10,11,12,13,14,6)): self.result_list.append_column(columns[c])
...
Right-click for more options""")
            ...
            self.result_list.connect("row-activated", self.parent.on_activate_result)
            self.result_list.connect("button-press-event", self._rbutton_press, 'contexts')
            ...
            self.result_list.set_tooltip_markup("""These are terms from read entries related to the one queries for.""")

            self.result_list.connect("button-press-event", self._rbutton_press, 'terms')
            ...
            self.result_list.connect("button-press-event", self._rbutton_press, 'time_series')
            ...
            self.result_list.set_tooltip_markup("""These are manually added rules used for flagging. 
Double-click to load rule as a query. 
Right-click for more options""")

            self.result_list.connect("button-press-event", self._rbutton_press, 'rules')
            self.result_list.connect("row-activated", self.parent.on_edit_rule)
    ...
    def on_close(self, *args, **kargs):
        """ Close window  """ 
        self.parent.remove_tab(self.tab_id)
    ...
    def reload_history(self, *args, **kargs):
        ...
        self.history.clear()
        hist = self.parent.FX.sqlite_cur.execute("select distinct string from search_history order by date DESC").fetchall()
    ...
    def on_date_changed(self, *args):
        ...
        if f_get_combo(self.qtime_combo) == 'choose_dates':
            ...
            if dialog.response == 1:
                ...
                model.append( (f'_{dialog.result["date_string"]}', dialog.result['date_string'])  )
...
""")
    ...
    def get_search_filters(self, **kargs):
        ...
        if time.startswith('_') and time != '__dummy':
            time = time[1:]
            dts = time.split(' --- ')
            if dts[0] != '...': self.search_filters['from_date'] = dts[0]
            if dts[1] != '...': self.search_filters['to_date'] = dts[1]
        else:
    ...
    def on_restore(self, *args, **kargs):
        ...
        f_set_combo(self.qfield_combo, self.parent.default_search_filters.get('field'), null_val=-1)
    ...
    def block_search(self, tab_text:str):
        ...
        if self.type in ('search','contexts','terms'):
    ...
    def finish_search(self):
        ...
        if (not self.parent.feed_win.processing_flag) and self.parent.curr_upper == self.tab_id and self.refresh_feeds: 
    ...
    def _query(self, qr, filters, **kargs):
        ...
        self.parent.processing_flags[self.tab_id] = True

        # DB interface for queries 
        FX = Feeder(config=self.config, debug=self.debug, ignore_images=True, gui=True, load_icons=False, ignore_lock=True, print=False)

        if self.type == 'search':

            self.final_status = f'Search for <b>{GObject.markup_escape_text(ellipsize(qr,50))}</b>'
            ...
            if qr == 'startup':
                ...
                if len(self.results) <= 1:
                    ...
                    if len(self.results) <= 1:
                        ...
                        if len(self.results) <= 1:
                            self.results = FX.QP.query('*', {'last_week':True, 'sort':'-importance'})

                self.final_status = 'News'


            elif filters == 'trash_bin':

                self.results = FX.QP.query('*', {'deleted_entries':True, 'sort':'-pubdate'}, no_history=True)
                self.final_status = 'Trash bin'

            elif filters != 'deleted':

                fltrs = {'sort':'-importance'}
                fltrs[filters] = True

                if filters == 'last_hour': self.final_status = 'Last Hour'
                elif filters == 'today': self.final_status = 'Today'
                elif filters == 'last_week': self.final_status = 'Last Week'
                elif filters == 'last_month': self.final_status = 'Last Month'
                elif filters == 'last_quarter': self.final_status = 'Last Quarter'
                elif filters == 'last_year': self.final_status = 'Last Year'
                elif filters == 'last': self.final_status = 'New Entries'

                self.results = FX.QP.query('*', fltrs, no_history=True)



        elif self.type == 'feed':
            self.final_status = f'<b>{GObject.markup_escape_text(ellipsize(qr, 50))}</b>'
            self.results = FX.QP.query('*', filters, no_history=True)

        elif self.type == 'similar':
            self.final_status = f'Similar to ...'
            self.results = FX.QP.find_similar(qr['id'], **filters)

        elif self.type == 'contexts':
            self.final_status = f'Contexts for <b>{GObject.markup_escape_text(ellipsize(qr, 50))}</b>'
            self.results = FX.QP.term_context(qr, **filters)

        elif self.type == 'terms':
            self.final_status = f'Terms related to <b>{GObject.markup_escape_text(ellipsize(qr,50))}</b>'
            self.results = FX.QP.term_net(qr, print=False, lang=filters.get('lang'), rev=True)

        elif self.type == 'time_series':
            self.final_status = f'Time Series for <b>{GObject.markup_escape_text(ellipsize(qr,50))}</b>'
            self.results = FX.QP.term_in_time(qr, **filters)


        if self.type == 'similar': 
            self._create_list(True, (0,), True, add_top=qr) 
        else: 
            self._create_list(True, (0,), True)
    ...
    def query(self, phrase, filters):
        ...
        self.block_search("Searching database...")
        self.refresh_feeds = True
        self.feed_filter_id = 0
    ...
    def on_query(self, *args):
        ...
        if self.type != 'terms': self.get_search_filters()
    ...
    def load_selection(self, *args, **kargs):
        ...
        if treeiter is None:
            if kargs.get('clear',False):
                if self.type in ('places','search','similar','feed'): self.parent.selection_res.clear()
                elif self.type =='contexts': self.parent.selection_res.clear()
                elif self.type =='terms': self.parent.selection_term = None
                elif self.type =='rules': self.parent.selection_rule.clear()
                
            if self.debug: print('Empty selection...')
            return -1
        

        if self.type == 'rules': id = model[treeiter][0]
        else: id = model[treeiter][1]

        for r in self.results:

            if self.type in ('places','search','similar','feed'):            
                if id == r[self.result.get_index('id')]:
                    self.parent.selection_res.populate(r)
                    self.parent.on_changed_selection(type=self.type)
                    if self.debug: print(f'Selection (res): {self.parent.selection_res["id"]}')
            elif self.type == 'contexts':
                if id == r[self.context.get_index('id')]:
                    self.parent.selection_res.populate(r[:-1])
                    self.parent.on_changed_selection(type=self.type)
                    if self.debug: print(f'Selection (context): {self.parent.selection_res["id"]}')

            elif self.type == 'terms':
                if id == r[0]:
                    self.parent.selection_term = id
                    if self.debug: print(f'Selection (term): {self.parent.selection_term}')

            elif self.type == 'rules':
                if id == r[self.rule.get_index('id')]:
                    self.parent.selection_rule.populate(r)
    ...
    def _create_list(self, feed_sum, feed_filter, ignore_filters, **kargs):
        ...
        if self.type in ('places','search','similar','trash','feed'):
            ...
            self.tmp_store = Gtk.ListStore(GdkPixbuf.Pixbuf, int,int, str,str,str,str,str,str,str,str,int,str,int, int,int, float,float,float, int,float,int,float, int,str, str)

            if kargs.get('add_top') != None:
                self.tmp_store.append(self._result_store_item(kargs.get('add_top')))
                self.tmp_store[0][24] = self.config.get('gui_new_color','#0FDACA')
                self.tmp_store[0][23] = 700
    ...
    def create_list(self, feed_sum, feed_filter, ignore_filters, **kargs):
        ...
        if self.processing_flag: return -1
        if len(self.result) == 0: return 0

        if kargs.get('results') != None:
            if self.type != 'rules': self.results = kargs.get('results').copy()
    ...
    def _result_store_item(self, result):
    ...
    def create_results_list(self, **kargs):
        ...
        feed_filter = kargs.get('feed_filter',False)
        ...
        for r in self.results:
            ...
            self.tmp_store.append(self._result_store_item(self.result))
    ...
    def _contexts_store_item(self, context):
    ...
    def create_contexts_list(self, **kargs):
        ...
        self.tmp_store = Gtk.ListStore(GdkPixbuf.Pixbuf, int,int, str,str,str,str,str, int,int,int, float,float,float,int,str, str)
        ...
        for r in self.results: 
            ...
            self.tmp_store.append(self._contexts_store_item(self.context))   
    ...
    def create_time_series(self, **kargs):
        ...
        self.tmp_store = Gtk.ListStore(str, str, int)
        ...
        for r in self.results:
            ...
            self.tmp_store.append((time, magn, count))
    ...
    def create_terms_list(self):
        """ Reload terms store"""
        self.tmp_store = Gtk.ListStore(str, float, int)
        ...
        for r in self.results:
            ...
            self.tmp_store.append( (term, weight, count) )
    ...
    def create_rules_list(self):
        """ Reload rules store """
        self.results = self.parent.FX.sqlite_cur.execute("""select * from rules where learned in (0,2) order by id desc""").fetchall()
        
        self.tmp_store = Gtk.ListStore(int, str, str, float, str, str, str, str, str, str, str)
        ...
        for r in self.results:
            ...
            self.tmp_store.append(( id, name, string, weight, case_ins, qtype, learned, flag, qfield, qfeed, color ))
    ...
    def apply_changes(self, id:int, action, **kargs):
        ...
        if self.type in ('places','search','similar','feed'): 
            ...
            if self.debug: print(f'Invalid type for tab uptate ({self.type})!')
            return -1


        if action in ('open', 'read','unread'):
            for i,r in enumerate(self.result_store):
                if r[1] == id:
                    if action == 'unread':
                        self.result_store[i][read_c] = 0
                        if weight_c != None: self.result_store[i][weight_c] = 400
                    else:
                        self.result_store[i][read_c] += 1
                        if weight_c != None: self.result_store[i][weight_c] = 700

            for i,r in enumerate(self.results):
                if r[0] == id:
                    self.results[i] = list(self.results[i])
                    if action == 'unread': self.results[i][self.result.get_index('read')] = 1 
                    else: self.results[i][self.result.get_index('read')] = coalesce(self.results[i][self.result.get_index('read')],0) + 1
            ...
            for i,r in enumerate(self.result_store):
                if r[1] == id:
                    if action == 'unflag':
                        self.result_store[i][flag_c] = 0
                        if weight_c != None: self.result_store[i][col_c] = None
                    else:
                        self.result_store[i][flag_c] = entry
                        if weight_c != None: self.result_store[i][col_c] = self._get_flag_color(entry)

            for i,r in enumerate(self.results):
                if r[0] == id: 
                    self.results[i] = list(self.results[i])
                    if action == 'unflag': self.results[i][self.result.get_index('flag')] = 0 
                    else: self.results[i][self.result.get_index('flag')] = entry
            ...
            for i,r in enumerate(self.result_store):
                if r[1] == id:
                    if self.type == 'places' and self.parent.curr_place == 'trash_bin': 
                        if action == 'restore': self.result_store[i][col_c] = self.config.get('gui_deleted_color','grey')
                        else: self.result_store[i][col_c] = 'red'
                    else:
                        if action == 'restore': self.result_store[i][col_c] = None
                        else: self.result_store[i][col_c] = self.config.get('gui_deleted_color','grey')
                
            for i,r in enumerate(self.results):
                if r[0] == id: 
                    self.results[i] = list(self.results[i])
                    if action == 'restore':  self.results[i][self.result.get_index('deleted')] = 0
                    else: self.results[i][self.result.get_index('deleted')] = 1

            if self.tab_id == self.parent.upper_pages[self.parent.curr_upper].tab_id: 
                self.parent.feed_win.reload_feeds()



        elif action == 'edit' and self.type != 'contexts':

            if entry == None: entry = list(self.parent.FX.sqlite_cur.execute(f"select {RESULTS_COLUMNS_SQL}\nwhere e.id = ?",(id,)).fetchone())
            ...
            for i,r in enumerate(self.result_store):
                if r[1] == id:
                    ...
                    self.result_store.insert(i, self._result_store_item(self.result))
                    break
            for i,r in enumerate(self.results):
                if r[0] == id:
                    self.results[i] = list(self.results[i])
                    self.results[i] = entry + [None,None]


        elif action == 'add' and self.type in ('places','search','feed'):
            ...
            new_result = list(self._result_store_item(self.result))
            new_result[col_c] = self.config.get('gui_new_color','#0FDACA')
