...
class FeedexTab(Gtk.ScrolledWindow):
    ...
    def __init__(self, parent, **kargs):
        ...
        if self.type == FX_TAB_SEARCH:
            ...
            self.header.set_markup('Flags')
        elif self.type == FX_TAB_REL_TIME:
            self.header.set_markup('Entry Relevance in Time')
        elif self.type == FX_TAB_TREE:
            self.header.set_markup('Summary')
        ...
        # Search bar
        if self.type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TERM_NET, FX_TAB_TIME_SERIES, FX_TAB_REL_TIME, FX_TAB_TREE):
            ...
            if self.type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TIME_SERIES, FX_TAB_REL_TIME, FX_TAB_TREE):
                ...
                if self.type in (FX_TAB_TIME_SERIES, FX_TAB_REL_TIME):
                    ...
                    self.group_combo = f_group_combo(ellipsize=False, tooltip='Select grouping field')
                    self.depth_combo = f_depth_combo(ellipsize=False, tooltip='Select how many top results to show for each grouping')
                    self.sort_combo = f_sort_combo(ellipsize=False, tooltip='Default sorting field for empty queries')
                ...
                if self.type != FX_TAB_REL_TIME:
                    ...
                    self.flag_combo         = f_flag_combo(self.parent.FX, connect=self.on_filters_changed, ellipsize=False, filters=True, tooltip='Filter by Flag or lack thereof')
                    ...
                    self.qfield_combo       = f_field_combo(connect=self.on_filters_changed, ellipsize=False, tooltip='Search in All or a specific field', all_label='-- No Field --')
        ...
        if self.type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_SIMILAR, FX_TAB_TREE):
            ...
            if self.type == FX_TAB_TREE: columns[4] = f_col('Title', 1, 4, color_col=24, attr_col=23, start_width=self.parent.gui_attrs.get('results',{}).get('4',650), name='4')
            else: columns[4] = f_col('Title', 0, 4, color_col=24, attr_col=23, start_width=self.parent.gui_attrs.get('results',{}).get('4',650), name='4') 
            ...
            columns[26] = f_col('Flag', 0, 26, color_col=24, attr_col=23, start_width=self.parent.gui_attrs.get('results',{}).get('26',100), name='26') 
            ...
            columns[15] = f_col('Flag ID', 0, 15, color_col=24, attr_col=23, start_width=self.parent.gui_attrs.get('results',{}).get('15',200), name='15')
...
Hit <b>Ctrl-F</b> for interactive search by Title""")
            ...
            self.result_list.append_column( f_col('', 3, 0, resizable=False, clickable=False, width=16, reorderable=False) )
            columns = {}
            columns[3] = f_col('Context', 1, 3, start_width=self.parent.gui_attrs.get('contexts',{}).get('3',650), name='3')
            columns[17] = f_col('Date', 0, 17, sort_col=8, start_width=self.parent.gui_attrs.get('contexts',{}).get('15',150), name='15')
            columns[4] = f_col('Source', 0, 4, start_width=self.parent.gui_attrs.get('contexts',{}).get('4',200), name='4')
            columns[5] = f_col('Title', 0, 5, start_width=self.parent.gui_attrs.get('contexts',{}).get('5',650), name='5')
            columns[19] = f_col('Flag', 1, 19, start_width=self.parent.gui_attrs.get('contexts',{}).get('19',150), name='19')
            columns[7] = f_col('Publ. Timestamp', 0, 7, start_width=self.parent.gui_attrs.get('contexts',{}).get('7',200), name='7')
            columns[9] = f_col('Read?', 0, 9, start_width=self.parent.gui_attrs.get('contexts',{}).get('9',200), name='9')
            columns[10] = f_col('Flag ID', 0, 10, start_width=self.parent.gui_attrs.get('contexts',{}).get('10',200), name='10')
            columns[11] = f_col('Importance', 0, 11, start_width=self.parent.gui_attrs.get('contexts',{}).get('11',200), name='11')
            columns[12] = f_col('Weight', 0, 12, start_width=self.parent.gui_attrs.get('contexts',{}).get('12',200), name='12')
            columns[13] = f_col('Readability', 0, 13, start_width=self.parent.gui_attrs.get('contexts',{}).get('13',200), name='13')
            columns[14] = f_col('Word Count', 0, 14, start_width=self.parent.gui_attrs.get('contexts',{}).get('14',200), name='14')
            columns[6] = f_col('Link', 0, 6, start_width=self.parent.gui_attrs.get('contexts',{}).get('6',400), name='6')
            columns[15] = f_col('Term Count', 0, 15, start_width=self.parent.gui_attrs.get('contexts',{}).get('15',200), name='15')
            columns[16] = f_col('Search Rank', 0, 16, start_width=self.parent.gui_attrs.get('contexts',{}).get('16',200), name='16')
...
Hit <b>Ctrl-F</b> for interactive search""")
            ...
            columns = {}
            columns[1] = f_col('Id', 0, 0, color_col=3, start_width=self.parent.gui_attrs.get('flags',{}).get('1',50), name='1')
            columns[2] = f_col('Name', 0, 1, color_col=3, start_width=self.parent.gui_attrs.get('flags',{}).get('2',200), name='2')
            columns[3] = f_col('Description', 0, 2, color_col=3, start_width=self.parent.gui_attrs.get('flags',{}).get('3',400), name='3')
            for c in self.parent.gui_attrs.get('flags_order', FX_DEF_COLS_FLAGS): self.result_list.append_column(columns[c])

            self.result_list.set_tooltip_markup("""Flags used in rules and for manual marking of Entries
Right-click for more options""")
            self.result_list.connect("button-press-event", self._rbutton_press, self.type)
...
""")
    ...
    def block_search(self, tab_text:str):
        ...
        if self.type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TERM_NET, FX_TAB_TIME_SERIES, FX_TAB_TREE):
    ...
    def _query(self, qr, filters, **kargs):
        ...
        if self.type == FX_TAB_SEARCH:
            ...
            self.final_status = f"Relevance in Time (<b>{self.top_entry['id']}</b>)..."            
            ...
            self.final_status = f'Summary <b>{esc_mu(qr, ell=50)}</b>'
            filters['rev'] = True
    ...
    def load_selection(self, *args, **kargs):
        ...
        if treeiter is None:
            if kargs.get('clear',False):
                if self.type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_SIMILAR, FX_TAB_TREE): self.parent.selection_res.clear()
    ...
    def _tree_group_item(self, result, res_idx, **kargs):
        ...
        icon = self.parent.icons.get(result["feed_id"],result['flag_name'])
    ...
    def _flags_store_item(self, flag, ix, **kargs):
        ...
        if kargs.get('deleted',False): color = self.config.get('gui_deleted_color','grey')
    ...
    def create_flags_list(self):
        ...
        self.result_store = self.tmp_store
        self.result_list.set_model(self.result_store)
        self.header.set_markup(f'Flags ({len(self.result_store)})')
    ...
    def _rules_store_item(self, rule, ix, **kargs):
        ...
        if self.rule['flag'] in (0,None): 
            ...
            flag = self.parent.FX.get_flag_name(self.rule['flag'])
            color = self.parent.FX.get_flag_color(self.rule['flag'])

        if kargs.get('deleted',False): color = self.config.get('gui_deleted_color','grey')

        return (id, name, string, weight, case_ins, qtype, learned, flag, qfield, qfeed, color, ix)
    ...
    def create_rules_list(self):
        ...
        self.result_store = self.tmp_store
        self.result_list.set_model(self.result_store)
        self.header.set_markup(f'Rules ({len(self.results)})')
    ...
    def apply_changes(self, entry, action, **kargs):
        ...
        if self.type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_SIMILAR, FX_TAB_CONTEXTS, FX_TAB_TREE):
            ...
            for ix,r in enumerate(self.results):
                if r[0] == id:
                    ...
                    if self.type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_SIMILAR, FX_TAB_TREE):
                        ...
                        self.results[ix] = self.rule.tuplify()
                    elif self.type == FX_TAB_FLAGS:
                    ...
                    if self.type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_SIMILAR):
                        ...
                        for ixx, rs in enumerate(self.result_store):
                            if rs[0] == id:
                                itr = self.result_store.get_iter(ixx)
                                self.result_store.remove(itr)
                                self.result_store.insert(ixx, self._rules_store_item(self.rule.tuplify(), ix))
                                break
                        break
            ...
            for ix, r in enumerate(self.results):
                if r[0] == id:
                    ...
                    for ixx, rs in enumerate(self.result_store):
                        if rs[0] == id:
                            itr = self.result_store.get_iter(ixx)
                            if self.type == FX_TAB_RULES: self.result_store[itr] = self._rules_store_item(self.results[ix], ix, deleted=True)
                            elif self.type == FX_TAB_FLAGS: self.result_store[itr] = self._flags_store_item(self.results[ix], ix, deleted=True)
                            break
                    break
