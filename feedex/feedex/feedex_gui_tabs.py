...
class FeedexTab(Gtk.ScrolledWindow):
    ...
    def __init__(self, parent, **kargs):
        ...
        # Search bar
        if self.type in ('search','contexts','terms','time_series','feed'):
            ...
            if self.type in ('search','contexts','time_series','feed'):
...
""")
    ...
    def block_search(self, tab_text:str):
        ...
        if self.type in ('search','contexts','terms','feed'):
    ...
    def _query(self, qr, filters, **kargs):
        ...
        if self.type in ('search','feed'):

            feed_name = f_get_combo(self.cat_combo, name=True)

            if qr in ('','*'):
                if feed_name is None: self.final_status = f'Results'
                else: self.final_status = f'<b>{GObject.markup_escape_text(ellipsize(feed_name,50))}</b>'
            else: 
                if feed_name is None: self.final_status = f'Search for <b>{GObject.markup_escape_text(ellipsize(qr,50))}</b>'
                else: self.final_status = f'Search for <b>{GObject.markup_escape_text(ellipsize(qr,50))}</b> in <b><i>{GObject.markup_escape_text(ellipsize(feed_name,20))}</i></b>'
