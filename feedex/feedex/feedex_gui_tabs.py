...
class FeedexTab(Gtk.ScrolledWindow):
    ...
    def __init__(self, parent, **kargs):
        ...
        # Search bar
        if self.type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TERM_NET, FX_TAB_TIME_SERIES, FX_TAB_REL_TIME):
            ...
            if self.type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TIME_SERIES, FX_TAB_REL_TIME):
...
""")
    ...
    def apply_changes(self, entry, action, **kargs):
        ...
        if self.type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_SIMILAR, FX_TAB_CONTEXTS):
            ...
            for ix,r in enumerate(self.results):
                if r[0] == id:
                    ...
                    if self.type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_SIMILAR):
                        ...
                        if self.type == FX_TAB_SEARCH: self.result['snippets'] = scast(self.results[ix][self.result.get_index('snippets')], list, []).copy()
                        self.results[ix] = self.result.tuplify()

                    elif self.type == FX_TAB_CONTEXTS:
                        self.context.merge(entry)
                        self.context['snippets'] = scast(self.results[ix][self.context.get_index('snippets')], list, []).copy()
