...
#
    ...
    def list_feeds(self, **kargs):
        ...
        if kargs.get("print",self.print):
            feeds = self.FX.qr_sql(FEEDS_PRINT_SQL, all=True)
            if self.FX.db_error is not None: return ()
            self.cli_table_print(FEEDS_SQL_TABLE_PRINT + ("Feedex Category",), feeds, mask=FEEDS_SHORT_PRINT, number_footnote='rergistered channels',
                            del_col=self.FX.feed.get_index('deleted'), passwd_col=self.FX.feed.get_index('passwd'))
        else: 
    ...
    def list_categories(self, **kargs):
        ...
        categories = self.FX.qr_sql(CATEGORIES_PRINT_SQL, all=True)
        if self.FX.db_error is not None: return ()
        if kargs.get("print",self.print):
    ...
    def read_entry(self, id:int, **kargs):
        ...
        if self.FX.db_error is not None: return ''
        if self.debug in (1,5) or kargs.get('to_var',False):
    ...
    def show_history(self, **kargs):
        ...
        self.results = self.FX.qr_sql(f'{SEARCH_HISTORY_SQL} asc', all=True)
        if self.FX.db_error is not None: return ()
        if kargs.get('print', self.print):
...
#
    ...
    def cli_table_print(self, columns:list, table, **kargs):
        ...
        if self.debug in (1,5): mask = columns
            ...
            for i in mask:
                ...
                if type(text) in (float, int):
                    ...
                    if output in ('cli', 'cli_noline'):
                        if spass:
                            text = '**********'
                        elif sdate and not self.debug in (1,5):
