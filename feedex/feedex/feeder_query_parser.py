...
or f.id in ( select ff.id from feeds ff where ff.parent_id = :feed_id {del_str} ))"""
    ...
    def query(self, string:str, filters:dict, **kargs):
        ...
        if category != None: 
            category = self.FX.resolve_category(category)
            if category != False: filters['category'] = category
            else: return -1


        feed = filters.get('feed')
        if feed != None:
            feed = self.FX.resolve_feed(feed)
            if feed != False: filters['feed'] = feed
            else: return -1  

        if filters.get('last',False): filters['from_added'] = self.FX.get_last()
        filters['field'] = field

        feed_or_cat = filters.get('feed_or_cat')
        if feed_or_cat != None:
            f_o_c = self.FX.resolve_category(feed_or_cat)
            if f_o_c == False:
                f_o_c = self.FX.resolve_feed(feed_or_cat)
                if f_o_c != False:
                    filters['feed'] = f_o_c
                else: return -1
            else: filters['category'] = f_o_c
        ...
        if filters.get('case_sens', None) == True: filters['case_ins'] = False
        ...
        if kargs.get('no_history',False) != True and phrase.get('emtpy',False) != True:
    ...
    def term_net(self, term:str, **kargs):
        ...
        if self.results in ((),[],None, (None,)):
            ...
            if self.results not in ((),[],None, (None,)):
                ...
                sql = f"select * from (select r.name as name, r.weight as weight, count(r.context_id) as c from rules r where r.context_id in ( {ids} ) group by r.name, r.weight ) t order by t.weight"
