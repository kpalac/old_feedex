        ...
        if kargs.get('print',self.print):
            ...
            if level in (2,3):
                ...
                if group is not None: 
                    self.results = self.group_results(self.results, group, depth)
                ...
                self.cli_table_print(RESULTS_SQL_TABLE_PRINT + node_header, self.results, flag_col=self.result.get_index('flag'),  
                                    del_col=self.result.get_index('deleted'), node_col=node_col, node_title_col=node_title_col,
...
Most similar read Entries:""")
    ...
    def group_results(self, table:list, group_by:str, depth:int, **kargs):
        ...
        if group_by not in ('category', 'feed', 'flag'): 
            self.FX.MC.ret_status = cli_msg( (-5, 'Invalid grouping! Must be: %a', 'category, feed or flag') )
            self.results = ()
            return ()
        ...
        if group_by == 'category':
            for f in feeds:
                if coalesce(f[feed.get_index('is_category')],0) == 1:
                    ...
                    feed.populate(f)
                    for r in table:
                        if r[self.result.get_index('parent_id')] == feed['id']:
                            count += 1
            ...
            for f in feeds:
                if coalesce(f[feed.get_index('is_category')],0) != 1:
                    ...
                    feed.populate(f)
                    for r in table:
                        if r[self.result.get_index('feed_id')] == feed['id']:
                            count += 1
                            node_tmp.append(list(r) + [0,])
                            if count >= depth: break
            ...
            for f in self.FX.MC.flags.keys():
                ...
                count = 0
                for r in table:
                    if r[self.result.get_index('flag')] == f:
                        count += 1
                        node_tmp.append(list(r) + [0,])
                        if count >= depth: break
        ...
        for entry in table:
            ...
            if output in ('cli', 'cli_noline','short','headlines'):
                ...
                if node_col != -1:
                    if scast(slist(entry, node_col, 0), int, 0)  == 1:
                        print(f'\n\n========= {scast(slist(entry, node_title_col, None), str, "<<UNKNOWN>>")} ===========================\n')
                        continue
            ...
            for i in mask:
                ...
                if type(text) in (float, int):
                    ...
                    if output in ('cli', 'cli_noline','short','headlines'): text = '<NONE>'
                    else: text = ''
                else:
                    text = scast(text, str, '')
                    if output in ('cli', 'cli_noline','short','headlines'):
