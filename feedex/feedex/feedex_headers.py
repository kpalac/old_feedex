...
select 
{RESULTS_COLUMNS_SQL}
where e.adddate >= :last and coalesce(f.is_category,0) = 0 and coalesce(f.deleted,0) <> 1
"""
...
RESULTS_SQL_TABLE_PRINT          = ENTRIES_SQL_TABLE_PRINT + ("Source (ID)", "Source", "Published - Timestamp", "Date", "Flag name", "User Agent", "Parent Category ID", "Parent Category", "Snippets", "Rank", "Count")
...
CONFIG_NAMES = {
            ...
            'default_depth' : 'Default grouping depth',
...
CONFIG_INTS_Z=('rule_limit','gui_clear_cache','default_depth')
