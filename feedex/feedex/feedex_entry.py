...
class EntryContainer(SQLContainerEditable):
    ...
    def get_by_id(self, id:int): 
        content = self.FX.curs.execute("select * from entries e where e.id = :id", {'id':id} ).fetchone()
    ...
    def get_by_url(self, url:str): 
        content = self.FX.curs.execute("select * from entries e where e.link = :url", {'url':url} ).fetchone()
        if content in (None, (None,), ()):
            self.exists = False
    ...
    def g_open(self, **kargs):
        ...
        if kargs.get('learn',True) and self.learn and self.config.get('use_keyword_learning', True) and read == 0:
            ...
            if err != 0: yield -2, 'DB error: %a', err
                ...
                if not self.FX.single_run: self.FX.load_rules()    
    ...
    def validate(self, **kargs):
        ...
        if self.vals.get('feed_or_cat') is not None:
            ...
            feed = self.FX.resolve_category(self.vals.get('category'), load=True)
            if feed == -1: return -1, 'Category %a not found!', self.vals.get('category', '<UNKNOWN>')
            self.feed.populate(feed)
            self.vals['feed_id'] = self.feed['id']
    ...
    def g_do_update(self, **kargs):
        ...
        if self.FX.rowcount > 0:
            ...
            if self.relearn and not self.FX.single_run: self.FX.load_rules()
            if restoring: self.FX.update_stats()
