...
class EntryContainer(SQLContainerEditable):
    ...
    def g_open(self, **kargs):
        """ Open in browser """
        if not self.exists:
            yield -8, 'Nothing to open. Aborting...'
            return -8
        ...
        if kargs.get('learn',True) and self.learn and self.config.get('use_keyword_learning', True) and read == 0:
            ...
            if err != 0: yield -2, 'DB error: %a', err
                ...
                if not self.FX.single_run: 
                    ...
                    if err != 0: 
                        yield -2, f'Error reloading rules after successfull action: %a', err
                        return -2                        
    ...
    def validate(self, **kargs):
        ...
        if self.vals.get('feed_or_cat') is not None:
            ...
            if feed == -1: return -7, 'Channel or Category %a not found!', self.vals.get('feed_or_cat', '<UNKNOWN>')
            ...
            if feed == -1: return -7, 'Channel or Category %a not found!', self.vals.get('feed_id', '<UNKNOWN>')
            self.feed.populate(feed)
            self.vals['feed_id'] = self.feed['id']

        elif self.vals.get('parent_id') is not None:
            feed = self.FX.resolve_f_o_c(self.vals.get('parent_id'), load=True)
            if feed == -1: return -7, 'Channel or Category %a not found!', self.vals.get('parent_id', '<UNKNOWN>')
            self.feed.populate(feed)
            self.vals['feed_id'] = self.feed['id']

        elif self.vals.get('parent_category') is not None:
            feed = self.FX.resolve_category(self.vals.get('parent_category'), load=True)
            if feed == -1: return -7, 'Category %a not found!', self.vals.get('parent_category', '<UNKNOWN>')
            self.feed.populate(feed)
            self.vals['feed_id'] = self.feed['id']
            ...
            if feed is None: return -7, 'Every entry needs to be assigned to a Channel or a Category!'
            if feed == -1: return -7, 'Channel %a not found!', self.vals['feed_id']
            self.feed.populate(feed)
            self.vals['feed_id'] = self.feed['id']


        if self.vals['flag'] not in (None, 0,1,2,3,4,5): return -7, 'Field %a needs to be in (None, 1-5)!', 'flag'
        ...
        if self.vals.get('link') is not None and not check_url(self.vals.get('link')):
            return -7, 'Not a valid url! (%a)', self.vals.get('link', '<UNKNOWN>')

        if self.vals['deleted'] not in (None, 0, 1): return -7, 'Deleted flag must be 0 or 1!'

        date = convert_timestamp(self.vals['pubdate_str'])
        if date is None: return -7, 'Invalid published date string (pubdate_str)!'
        else: self.vals['pubdate'] = date

        if self.vals['adddate_str'] is not None:
            date = convert_timestamp(self.vals['adddate_str'])
            if date is None: return -7, 'Invalid adding date string (%a)!', self.vals['adddate_str']
    ...
    def g_do_update(self, **kargs):
        ...
        if not self.updating: 
            ...
            return 0
        if not self.exists: 
            yield -8, 'Nothing to do. Aborting...'
            return -8
        ...
        if self.FX.rowcount > 0:
            ...
            if self.relearn and not self.FX.single_run: 
                err = self.FX.load_rules()
                if err != 0: 
                    yield -2, f'Error reloading rules after successfull update: %a', err
                    return -2
            if restoring: 
                err = self.FX.update_stats()
                if err != 0: 
                    yield -2, f'Error updating DB stats after successfull update: %a', err
                    return -2
    ...
    def r_delete(self, **kargs):
        ...
        if id is None: return -8, 'Entry %a not found!', id
    ...
    def g_add(self, **kargs):
        ...
        if kargs.get('update_stats',True):
            ...
            err = self.FX.update_stats()
            if err != 0: return -2, f'Error updating DB stats after successfull add: %a', err
