...
class EntryContainer(SQLContainerEditable):
    ...
    def g_open(self, **kargs):
        ...
        if kargs.get('update_read',True):
            err = self.FX.run_sql_lock('update entries set read = coalesce(read,0) + 1 where id = :id', {'id':self.vals['id']})
            if err != 0: 
    ...
    def g_do_update(self, **kargs):
        ...
        for f in self.to_update:
            if f in LING_LIST1: self.recalculate = True
        ...
        if self.FX.rowcount > 0:
            ...
            if len(self.to_update) > 1:
                ...
                for f in self.to_update:
                    ...
                    elif f in ('feed_id','feed','category','feed_or_cat'): 
