...
class EntryContainer(SQLContainerEditable):
    ...
    def get_by_id(self, id:int): 
        ...
        if content in (None, (None,), ()):
            self.exists = False
            cli_msg( (-1, 'Entry %a not found!', id) )
            return -1
        else:
            self.exists = True
    ...
    def get_by_url(self, url:str): 
        ...
        if content in (None, (None,), ()):
            self.exists = False
            cli_msg( (-1, 'Entry %a not found!', url) )
            return -1
        else:
            self.exists = True
            self.populate(content)
    ...
    def open(self, **kargs):
        for msg in self.g_open(**kargs): cli_msg(msg)
    def g_open(self, **kargs):
        """ Open in browser """
        if not self.exists:
            yield -1, 'Nothing to open!'
            return -1

        read = coalesce(self.vals['read'],0)
        if kargs.get('update_read',True):
            if read >= 0: err = self.FX.run_sql_lock('update entries set read = coalesce(read,0) + 1 where id = :id', {'id':self.vals['id']})
            else: err = self.FX.run_sql_lock('update entries set read = 1 where id = :id', {'id':self.vals['id']})
            if err != 0: 
        ...
        if self.vals.get('link') is not None:
            ...
            for idx, arg in enumerate(command):
                if arg in ('%u', '%U', '%f', '%F'):
                    command[idx] = self.vals.get('link','')

            if self.debug: print(' '.join(command))
            yield 0, 'Opening in browser (%a) ...', self.vals.get('link', '<UNKNOWN>')
        ...
        if kargs.get('learn',True) and self.learn and self.config.get('use_keyword_learning', True) and read == 0:
            ...
            if err != 0: yield -2, 'DB error: %a', err
                ...
                yield 0, 'Keywords learned'
                if not self.FX.single_run: 
                    err = self.FX.load_rules()
                    if err != 0: 
                        yield -1, f'Error reloading rules after successfull action: %a', err
                        return -1                        
    ...
    def validate(self, **kargs):
        ...
        if err_fld != 0: return -1, 'Invalid type for field %a', err_fld

        if self.vals.get('feed_or_cat') is not None:
            feed = self.FX.resolve_f_o_c(self.vals.get('feed_or_cat'), load=True)
            if feed == -1: return -1, 'Channel or Category %a not found!', self.vals.get('feed_or_cat', '<UNKNOWN>')
            ...
            if feed == -1: return -1, 'Channel or Category %a not found!', self.vals.get('feed_id', '<UNKNOWN>')
            self.feed.populate(feed)
            self.vals['feed_id'] = self.feed['id']

        elif self.vals.get('parent_id') is not None:
            feed = self.FX.resolve_f_o_c(self.vals.get('parent_id'), load=True)
            if feed == -1: return -1, 'Channel or Category %a not found!', self.vals.get('parent_id', '<UNKNOWN>')
            self.feed.populate(feed)
            self.vals['feed_id'] = self.feed['id']

        elif self.vals.get('parent_category') is not None:
            feed = self.FX.resolve_category(self.vals.get('parent_category'), load=True)
            if feed == -1: return -1, 'Category %a not found!', self.vals.get('parent_category', '<UNKNOWN>')
            self.feed.populate(feed)
            self.vals['feed_id'] = self.feed['id']
            ...
            if feed is None: return -1, 'Every entry needs to be assigned to a Channel or a Category!'
            if feed == -1: return -1, 'Channel %a not found!', self.vals['feed_id']
            self.feed.populate(feed)
            self.vals['feed_id'] = self.feed['id']


        if self.vals['flag'] not in (None, 0,1,2,3,4,5): return -1, 'Field %a needs to be in (None, 1-5)!', 'flag'
        #if coalesce(self.vals['read'],0) < 0: return -1, 'Field %a must be a positive integer!', 'read'

        if self.vals.get('link') is not None and not check_url(self.vals.get('link')):
            return -1, 'Not a valid url! (%a)', self.vals.get('link', '<UNKNOWN>')

        if self.vals['deleted'] not in (None, 0, 1): return -1, 'Deleted flag must be 0 or 1!'

        date = convert_timestamp(self.vals['pubdate_str'])
        if date is None: return -1, 'Invalid published date string (pubdate_str)!'
        else: self.vals['pubdate'] = date

        if self.vals['adddate_str'] is not None:
            date = convert_timestamp(self.vals['adddate_str'])
            if date is None: return -1, 'Invalid adding date string (%a)!', self.vals['adddate_str']
    ...
    def do_update(self, **kargs): 
        for msg in self.g_do_update(**kargs): cli_msg(msg)
    def g_do_update(self, **kargs):
        """ Apply edit changes to DB """
        if not self.updating: 
            yield 0, 'Nothing to do'
            return 0
        if not self.exists: 
            yield -1, 'Nothing to edit. Aborting'
            return -1
        ...
        if self.learn and self.config.get('use_keyword_learning', True):
            if self.recalculate and coalesce(self.vals['read'],0) > 1: self.relearn = True
        ...
        if self.recalculate or self.relearn: 
            if self.recalculate:  yield 0, 'Recalculating statistics ...'
            if self.relearn: yield 0, 'Extracting and learning keywords ...'

            err = self.ling(index=True, stats=self.recalculate, learn=self.relearn, save_rules=self.relearn)
            if err != 0: 
                self.restore()
                yield -1, 'Error processing linguistic data: %a', err
                return -1
        ...
        if self.FX.rowcount > 0:
            ...
            if self.relearn and not self.FX.single_run: 
                err = self.FX.load_rules()
                if err != 0: 
                    yield -1, f'Error reloading rules after successfull update: %a', err
                    return -1
            if restoring: 
                err = self.FX.update_stats()
                if err != 0: 
                    yield -1, f'Error updating DB stats after successfull update: %a', err
                    return -1

            if len(self.to_update) > 1:
                self.FX.log(False, f'Entry {self.vals["id"]} updated')
                yield 0, 'Entry %a updated successfuly!', self.vals['id']
            else:
                for f in self.to_update:
                    if f == 'read' and self.vals.get(f,0) > 0: 
                        self.FX.log(False, f'Entry {self.vals["id"]} marked as read')
                        yield 0, 'Entry %a marked as read', self.vals['id']
                        ...
                        yield 0, 'Entry %a marked as unread', self.vals['id']
                    elif f == 'flag' and self.vals.get(f) in (0,None): 
                        self.FX.log(False, f'Entry {self.vals["id"]} unflagged')
                        yield 0, 'Entry %a unflagged', self.vals['id']
                    elif f == 'flag' and self.vals.get(f) not in (0, None): 
                        self.FX.log(False, f'Entry {self.vals["id"]} flagged')
                        yield 0, 'Entry %a flagged', self.vals['id']
                    elif f == 'deleted' and self.vals.get(f) in (0, None): 
                        self.FX.log(False, f'Entry {self.vals["id"]} restored')
                        yield 0, 'Entry %a restored', self.vals['id']
                    elif f == 'feed_id': 
                        self.FX.log(False, f'Entry {self.vals["id"]} assigned to {self.feed.name(id=True)}')
                        yield 0, f'Entry %a assigned to {self.feed.name(id=True)}', self.vals['id']
                    else:
                        self.FX.log(False, f'Entry {self.vals["id"]} updated:  {f} -> {self.vals.get(f,"<NULL>")}')
                        yield 0, f'Entry %a updated:  {f} -> {self.vals.get(f,"<NULL>")}', self.vals['id']
            return 0
    ...
    def update(self, idict, **kargs): 
        for msg in self.g_update(idict, **kargs): cli_msg(msg)
    def g_update(self, idict, **kargs):
        """ Quick update with a value dictionary"""
        if not self.exists:
            yield -1, 'Entry not found'
            return 0
    ...
    def delete(self, **kargs): cli_msg(self.r_delete(**kargs))
    def r_delete(self, **kargs):
        """ Delete from DB with rules if necessary """
        if not self.exists: return -1, 'Entry not found'
        ...
        if id is None: return -1, 'Entry %a not found!', id
        ...
        if self.vals.get('deleted',0) == 1:
            ...
            err = self.FX.run_sql_lock("update entries set deleted = 1 where id = :id", {'id':id} )
        ...
        if self.FX.rowcount > 0:
            ...
            if not self.FX.single_run: 
                err = self.FX.load_rules()
                if err != 0: return -1, f'Error reloading rules after successfull delete: %a', err
            err = self.FX.update_stats()
            if err != 0: return -1, f'Error updating DB stats after successfull delete: %a', err
    ...
    def add(self, **kargs):
        for msg in self.g_add(**kargs): cli_msg(msg)
    def g_add(self, **kargs):
        ...
        if learn:
            yield 0, 'Extracting and learning keywords ...'
            err = self.learn_rules(self.FX.LP.features)
            if err != 0:
                yield -1, 'Error saving keywords: %a', err
                return -1
            else:
                yield 0, 'Keywords learned for entry %a', self.vals['id']

        if kargs.get('update_stats',True):
            if not self.FX.single_run: 
                err = self.FX.load_rules()
                if err != 0: return -1, f'Error reloading rules after successfull add: %a', err

            err = self.FX.update_stats()
            if err != 0: return -1, f'Error updating DB stats after successfull add: %a', err

        yield 0, 'Entry %a added', self.vals['id']
        return 0
    ...
    def relearn_keywords(self, **kargs):
        ...
        if not self.exists: return 'Entry not found!'
        if not self.config.get('use_keyword_learning', True): return 0
        return self.ling(learn=True, save_rules=kargs.get('save_rules', True))

    def rerank(self, **kargs): 
        if not self.exists: return 'Entry not found!'
        return self.ling(rank=True)

    def reindex(self, **kargs): 
        if not self.exists: return 'Entry not found!'
        return self.ling(index=True, stats=True)
    ...
    def learn_rules(self, features:dict, **kargs):
        ...
        self.rules.clear()
        if self.debug: print("Consolidating extracted features...")
