...
class FeedContainer(FeedContainerBasic):
    ...
    def validate_regexes(self, **kargs):
        ...
        for f in FEEDS_REGEX_HTML_PARSERS:
            if self.vals[f] is not None and not check_if_regex(self.vals[f]):
                return -7, f'HTML parser: Not a valid REGEX ({f}): %a', self.vals[f]
        return 0
    ...
    def validate(self, **kargs):
        ...
        if self.vals['is_category'] == 1:
            if self.vals['parent_id'] not in (None, 0) and self.vals['parent_id'] == self.vals['id']: return -7, 'Nested categories are not allowed!'
            if self.vals.get('parent_category') not in (None, 0): return -7, 'Nested categories are not allowed!'
        else:
            if self.vals['handler'] in ('rss','html') and not check_url(self.vals['url']): return -7, 'Not a valid URL! (%a)', self.vals['url']
            ...
            if self.vals['handler'] not in ('rss','local','html','script'): return -7, 'Invalid handler! Must be rss, local or html'
            ...
            if self.vals['fetch'] not in (None, 0, 1): return -7, 'Fetch flag must be 0 or 1!'
            ...
            if len(self.to_update) > 1:
                ...
                for f in self.to_update:
                    ...
                    if f == 'parent_id' and self.vals[f] not in (None, 0): 
                        ...
                        return 0, f'Fetching disabled for %a', self.name(id=True)
                    elif f == 'fetch' and self.vals[f] == 1:
                        self.FX.log(False, f'Fetching enabled for {self.name(id=True)}')
                        return 0, f'Fetching enabled for %a', self.name(id=True)
    ...
    def r_update_feed_order(self, **kargs):
        ...
        if err != 0: return -2, 'DB error while updating feed list order: %a', err
        return 0
    ...
    def r_order_insert(self, target:int, **kargs):
        ...
        if not target.exists: return -8, 'Target by id %a not found', target
        if not self.exists: return -8, 'Channel/Category %a not found', self.vals.get('id')
        ...
        if (self.vals['is_category'] == 1 and target['is_category'] == 1) or (self.vals['is_category'] != 1 and target['is_category'] != 1):
            ...
            except (ValueError, IndexError) as e: return -7, 'Error insering Channel/Category: %a', e
        ...
        if not self.FX.single_run:
            err = self.FX.load_feeds()
            if err != 0: return -2, f'Error reloading Feeds after successfull operation: %a', err

        return 0, f'Display order changed successfully...'