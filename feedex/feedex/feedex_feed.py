...
class FeedContainer(FeedContainerBasic):
    ...
    def validate(self, **kargs):
        ...
        if self.vals['is_category'] == 1:
            ...
            if self.vals['handler'] not in ('rss','local','html','script'): return -7, _('Invalid handler! Must be rss, local or html')
            ...
            if self.vals['auth'] is not None:
                if self.vals['auth'] not in ('detect','basic','digest'): return -7, _('Invalid authentication methon (must be NONE, detect, basic or digest)')
