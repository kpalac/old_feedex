...
class FeedContainer(FeedContainerBasic):
    ...
    def open(self, **kargs): cli_msg(self.open(**kargs))
    def r_open(self, **kargs):
        """ Open in browser """
        if not self.exists: return -1, 'Nothing to open!'
        if self.vals['is_category'] == 1: return -1, 'Cannot open category!'
        ...
        return 0, 'Sent to browser (%a)', self.vals.get('link', '<UNKNOWN>')
    ...
    def validate(self, **kargs):
        ...
        if err != 0: return -1, 'Invalid data type for %a', err

        if self.vals['is_category'] == 1:
            if self.vals['parent_id'] not in (None, 0): return -1, 'Nested categories are not allowed!'
            if self.vals.get('parent_category') not in (None, 0): return -1, 'Nested categories are not allowed!'
        else:
            if self.vals['handler'] in ('rss',) and not check_url(self.vals['url']): return -1, 'Not a valid URL! (%a)', self.vals['url']
            if self.vals['link'] is not None and not check_url(self.vals['link']): return -1, 'Not a valid URL! (%a)', self.vals['link']
            ...
            if 'parent_category' in self.vals.keys():
                if self.vals.get('parent_category') is not None:
                    ...
                    if feed == -1: return -1, 'Category %a not found!', self.vals['parent_category']
                ...
                if self.vals['parent_id'] is not None:
                    ...
                    if feed == -1: return -1, 'Category %a not found!', self.vals['parent_id']
                    self.parent_feed.populate(feed)
                    self.vals['parent_id'] = self.parent_feed['id']
                else: self.vals['parent_id'] = None


            if self.vals['handler'] not in ('rss','local','twitter'): return -1, 'Invalid handler!'
            if self.vals['interval'] is None or self.vals['interval'] < 0: return -1, 'Interval must be >= 0!'
            if self.vals['autoupdate'] not in (None, 0, 1): return -1, 'Autoupdate flag must be 0 or 1!'
        
        if self.vals['deleted'] not in (None, 0, 1): return -1, 'Daleted flag must be 0 or 1!'
    ...
    def do_update(self, **kargs): cli_msg(self.r_do_update(**kargs)) 
    def r_do_update(self, **kargs):
        """ Apply edit changes to DB """
        if not self.updating: return 0, 'Nothing to do'
        if not self.exists: return 0, 'Channel/Category not found. Aborting'
        ...
        if self.FX.rowcount > 0:
            ...
            if not self.FX.single_run: 
                ...
                if err != 0: return -1, f'Error reloading Feeds after successfull update: %a', err

            if restoring:
                err = self.FX.update_stats()
                if err != 0: return -1, f'Error updating DB stats after successfull update: %a', err
                err = self.FX.load_rules()
                if err != 0: return -1, f'Error reloading rules after successfull update: %a', err
    ...
    def update(self, idict, **kargs): cli_msg(self.r_update(idict, **kargs))
    def r_update(self, idict, **kargs):
        """ Quick update with a value dictionary """
        if not self.exists: return 0, 'Nothing found'
    ...
    def r_delete(self, **kargs):
        ...
        if self.FX.rowcount > 0:
            ...
            if not self.FX.single_run: 
                ...
                if err != 0: return -1, f'Error reloading data after successfull delete: %a', err
    ...
    def add(self, **kargs): cli_msg(self.r_add(**kargs))
    def r_add(self, **kargs):
        ...
        if not self.FX.single_run: 
            err = self.FX.load_feeds()
            if err != 0: return -1, f'Error reloading Feeds after successfull add: %a', err
    ...
    def add_from_url(self, **kargs): 
        for msg in self.g_add_from_url(**kargs): cli_msg(msg)
    def g_add_from_url(self, **kargs):
        ...
        for f in self.FX.MC.feeds:
            if f[self.get_index('url')] == self.vals['url'] and f[self.get_index('deleted')] != 1:
                yield -1, 'Channel with this URL already exists (%a)', f'{f[self.get_index("name")]} ({f[self.get_index("id")]})'
                return -1
