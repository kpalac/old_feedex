...
class FeedexMainWin(Gtk.Window):
    ...
    def __init__(self, feedex_main_container, **kargs):
        ...
        self.export_menu.append( f_menu_item(1, 'Export Feed data to JSON', self.export_feeds, icon='application-rss+xml-symbolic'))  
        self.export_menu.append( f_menu_item(1, 'Export Rules to JSON', self.export_rules, icon='view-list-compact-symbolic'))  
        self.export_menu.append( f_menu_item(1, 'Export Flags to JSON', self.export_flags, icon='marker-symbolic'))  
        self.export_menu.show_all()

        self.import_menu = Gtk.Menu()
        self.import_menu.append( f_menu_item(1, 'Import Feed data from JSON', self.import_feeds, icon='application-rss+xml-symbolic'))  
        self.import_menu.append( f_menu_item(1, 'Import Rules data from JSON', self.import_rules, icon='view-list-compact-symbolic'))  
        self.import_menu.append( f_menu_item(1, 'Import Flags data from JSON', self.import_flags, icon='marker-symbolic'))  
        self.import_menu.show_all()

        self.db_menu = Gtk.Menu()
        self.db_menu.append( f_menu_item(1, 'Archive...', self.on_archive, icon='archive-symbolic', tooltip="""Archive entries older than certain time threshold to a separate DB.
It can be then packed or deleted"""))  
        self.db_menu.append( f_menu_item(1, 'Maintenance...', self.on_maintenance, icon='preferences-system-symbolic', tooltip="""Maintenance can improve performance for large databases by doing cleanup and reindexing.
It will also take some time to perform"""))  
        self.db_menu.append( f_menu_item(0, 'SEPARATOR', None) )
        self.db_menu.append( f_menu_item(1, 'Database statistics', self.on_show_stats, icon='drive-harddisk-symbolic') )


        # Main Menu
        self.main_menu = Gtk.Menu()
        self.main_menu.append( f_menu_item(1, 'Rules', self.add_tab, kargs={'type':FX_TAB_RULES}, icon='view-list-compact-symbolic', tooltip='Open a new tab showing Saved Rules') )
        self.main_menu.append( f_menu_item(1, 'Flags', self.add_tab, kargs={'type':FX_TAB_FLAGS}, icon='marker-symbolic', tooltip='Open a new tab showing Flags') )
        ...
        self.main_menu.append( f_menu_item(0, 'SEPARATOR', None) )
        self.main_menu.append( f_menu_item(3, 'Export...', self.export_menu, icon='document-export-symbolic'))  
        self.main_menu.append( f_menu_item(3, 'Import...', self.import_menu, icon='document-import-symbolic'))  
        self.main_menu.append( f_menu_item(0, 'SEPARATOR', None) )
        self.main_menu.append( f_menu_item(3, 'Database...', self.db_menu, icon='drive-harddisk-symbolic') )
        self.main_menu.append( f_menu_item(0, 'SEPARATOR', None) )
        ...
        self.search_menu.append( f_menu_item(1, 'Search', self.add_tab, kargs={'type': FX_TAB_SEARCH}, icon='edit-find-symbolic', tooltip='Search entries'))  
        self.search_menu.append( f_menu_item(1, 'Summary', self.add_tab, kargs={'type': FX_TAB_TREE}, icon='view-filter-symbolic', tooltip='Search entries grouping them in a tree summary'))  
        self.search_menu.append( f_menu_item(0, 'SEPARATOR', None) )
        self.search_menu.append( f_menu_item(1, 'Show Contexts for a Term', self.add_tab, kargs={'type': FX_TAB_CONTEXTS}, icon='view-list-symbolic', tooltip='Search for Term Contexts'))  
        self.search_menu.append( f_menu_item(1, 'Show Time Series for a Term', self.add_tab, kargs={'type': FX_TAB_TIME_SERIES}, icon='office-calendar-symbolic', tooltip='Generate time series plot'))  
        self.search_menu.append( f_menu_item(0, 'SEPARATOR', None) )
        self.search_menu.append( f_menu_item(1, 'Search for Related Terms', self.add_tab, kargs={'type': FX_TAB_TERM_NET}, icon='emblem-shared-symbolic', tooltip='Search for Related Terms from read/opened entries'))  
        ...
        self.hbar_button_menu.set_tooltip_markup("""Main Menu""")
        ...
        self.prev_enc   = f_label(None, justify=FX_ATTR_JUS_CENTER, selectable=True, wrap=True, markup=True)
    ...
    def _rbutton_press(self, widget, event, from_where):
        ...
        if event.button == 3:
            ...
            if from_where in (FX_TAB_RESULTS, FX_TAB_CONTEXTS, FX_TAB_TREE):
                menu = Gtk.Menu()
                ...
                if self.selection_res['id'] is not None:
                    ...
                    if (not self.flag_edit_entry) and self.selection_res['deleted'] != 1 and not (self.curr_upper == 0 and self.curr_place == FX_PLACE_TRASH_BIN):
                        ...
                        mark_menu = Gtk.Menu()
                        mark_menu.append( f_menu_item(1, 'Read (+1)', self.on_mark_recalc, args=('read',), icon='bookmark-new-symbolic', tooltip="Number of reads if counted towards this entry keyword's weight when ranking incoming articles ") )
                        mark_menu.append( f_menu_item(1, 'Unread', self.on_mark, args=('unread',), icon='edit-redo-rtl-symbolic', tooltip="Unread document does not contriute to ranking rules") )
                        mark_menu.append( f_menu_item(1, 'Unimportant', self.on_mark_recalc, args=('unimp',), icon='edit-redo-rtl-symbolic', tooltip="Mark this as unimportant and learn negative rules") )

                        menu.append( f_menu_item(3, 'Mark as...', mark_menu, icon='bookmark-new-symbolic') )
                        ...
                        for fl, v in self.FX.MC.flags.items():
                            ...
                            if fl_name in (None, ''): fl_name = f'Flag {fl}'
                            flag_menu.append( f_menu_item(1, fl_name, self.on_mark, args=(fl,), color=fl_color, icon='marker-symbolic') )
                        flag_menu.append( f_menu_item(1, 'Unflag Entry', self.on_mark, args=('unflag',), icon='edit-redo-rtl-symbolic', tooltip="Remove Flags from Entry") )

                        menu.append( f_menu_item(3, 'Flag...', flag_menu, icon='marker-symbolic', tooltip="Flag is a user's marker/bookmark for a given article independent of ranking\n<i>You can setup different flag colors in Preferences</i>") )
                    ...
                    similar_menu.append( f_menu_item(1, 'Last update', self.on_from_entry, args=('last',FX_TAB_SIMILAR) ) )
                    similar_menu.append( f_menu_item(1, 'Today', self.on_from_entry, args=('today',FX_TAB_SIMILAR) ) )
                    similar_menu.append( f_menu_item(1, 'Last Week', self.on_from_entry, args=('last_week',FX_TAB_SIMILAR) ) )
                    similar_menu.append( f_menu_item(1, 'Last Month', self.on_from_entry, args=('last_month',FX_TAB_SIMILAR) ) )
                    similar_menu.append( f_menu_item(1, 'Last Quarter', self.on_from_entry, args=('last_quarter',FX_TAB_SIMILAR) ) )
                    similar_menu.append( f_menu_item(1, 'Last Year', self.on_from_entry, args=('last_year',FX_TAB_SIMILAR) ) )
                    similar_menu.append( f_menu_item(1, 'Select Range...', self.on_from_entry, args=('range',FX_TAB_SIMILAR) ) )
                        
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
                    menu.append( f_menu_item(3, 'Find Similar Entries', similar_menu, icon='edit-copy-symbolic', tooltip="Find Entries similar to the one selected") )
                    
                    menu.append( f_menu_item(1, 'Show Time Relevance', self.add_tab, kargs={'type': FX_TAB_REL_TIME}, icon='office-calendar-symbolic', tooltip="Search for this Entry's Keywords in time") )

                    author_menu = Gtk.Menu()
                    author_menu.append( f_menu_item(1, 'Articles', self.add_tab, kargs={'type': FX_TAB_SEARCH, 'author': True}, icon='edit-find-symbolic', tooltip="Search for other documents by this Author" ) )
                    author_menu.append( f_menu_item(1, 'Activity in Time', self.add_tab, kargs={'type': FX_TAB_TIME_SERIES, 'author': True}, icon='office-calendar-symbolic', tooltip="Search for other documents by this Author in Time Series" ) )
                    author_menu.append( f_menu_item(0, 'SEPARATOR', None) )
                    author_menu.append( f_menu_item(1, 'Search WWW', self._on_www_search_auth, icon='www-symbolic', tooltip="Search WWW for this Author's info" ) )
                    
                    menu.append( f_menu_item(3, 'Other by this Author...', author_menu, icon='emblem-shared-symbolic', tooltip="Search for this Author") )

                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
                    
                    menu.append( f_menu_item(1, 'Show Keywords for Entry', self.on_show_keywords, icon='zoom-in-symbolic') )
                    menu.append( f_menu_item(1, 'Show Matched Rules for Entry', self.on_show_rules_for_entry, icon='zoom-in-symbolic') )
                    if self.debug is not None: menu.append( f_menu_item(1, 'Details...', self.on_show_detailed, icon='zoom-in-symbolic', tooltip="Show all entry's technical data") )
                ...
                if self.upper_pages[self.curr_upper].type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TREE):
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
                    menu.append( f_menu_item(1, 'Clear Search History', self.on_clear_history, icon='edit-clear-symbolic') )
                
                if self.curr_upper == 0 and self.curr_place == FX_PLACE_TRASH_BIN: menu.append( f_menu_item(1, 'Empty Trash', self.on_empty_trash, icon='edit-delete-symbolic') ) 
                menu.append( f_menu_item(0, 'SEPARATOR', None) )
                menu.append( f_menu_item(1, 'Save layout', self.upper_pages[self.curr_upper].save_layout, icon='document-page-setup-symbolic', tooltip='Save column layout and sizing for current tab.\nIt will be used as default in the future' ) )
                if self.upper_pages[self.curr_upper].type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TREE):
                    menu.append( f_menu_item(1, 'Save filters', self.upper_pages[self.curr_upper].save_filters, icon='view-column-symbolic', tooltip='Save current search filters as defaults for future' ) )

                if self.upper_pages[self.curr_upper].type == FX_TAB_TREE:
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
                    menu.append( f_menu_item(1, 'Expand all', self.upper_pages[self.curr_upper]._expand_all, icon='list-add-symbolic') )
                    menu.append( f_menu_item(1, 'Collapse all', self.upper_pages[self.curr_upper]._collapse_all, icon='list-remove-symbolic') )
                ...
                if self.selection_rule['id'] is not None:
                    ...
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
                    menu.append( f_menu_item(1, 'Search for this Rule', self.add_tab, kargs={'type': FX_TAB_SEARCH, 'phrase':self.selection_rule['string'], 'qtype':self.selection_rule['type'], 'case_ins':self.selection_rule['case_insensitive']}, icon='edit-find-symbolic'))  
                    menu.append( f_menu_item(1, 'Show this Term\'s Contexts', self.add_tab, kargs={'type': FX_TAB_CONTEXTS, 'phrase':self.selection_rule['string'], 'qtype':self.selection_rule['type'], 'case_ins':self.selection_rule['case_insensitive']}, icon='view-list-symbolic'))  
                    menu.append( f_menu_item(1, 'Show Terms related to this Term', self.add_tab, kargs={'type': FX_TAB_TERM_NET, 'phrase':self.selection_rule['string']}, icon='emblem-shared-symbolic'))  
                    menu.append( f_menu_item(1, 'Show Time Series for this Term', self.add_tab,kargs={'type': FX_TAB_TIME_SERIES, 'phrase':self.selection_rule['string'], 'qtype':self.selection_rule['type'], 'case_ins':self.selection_rule['case_insensitive']}, icon='office-calendar-symbolic'))  
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
                ...
                if self.selection_term not in (None,''):
                    menu.append( f_menu_item(1, 'Search for this Term', self.add_tab, kargs={'type': FX_TAB_SEARCH, 'phrase':self.selection_term}, icon='edit-find-symbolic'))  
                    menu.append( f_menu_item(1, 'Show this Term\'s Contexts', self.add_tab, kargs={'type': FX_TAB_CONTEXTS, 'phrase':self.selection_term}, icon='view-list-symbolic'))  
                    menu.append( f_menu_item(1, 'Show Terms related to this Term', self.add_tab, kargs={'type': FX_TAB_TERM_NET, 'phrase':self.selection_term}, icon='emblem-shared-symbolic'))  
                    menu.append( f_menu_item(1, 'Show Time Series for this Term', self.add_tab,kargs={'type': FX_TAB_TIME_SERIES, 'phrase':self.selection_term}, icon='office-calendar-symbolic'))  
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
                ...
                if self.selection_time_series not in (None, {}):
                    menu.append( f_menu_item(1, 'Search this Time Range', self.add_tab, kargs={'type': FX_TAB_SEARCH, 'date_range':True}, icon='edit-find-symbolic'))  
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )  
                menu.append( f_menu_item(1, 'Save results to CSV', self.export_results_csv, icon='x-office-spreadsheet-symbolic', tooltip='Save results from current tab'))  
                menu.append( f_menu_item(0, 'SEPARATOR', None) )
                menu.append( f_menu_item(1, 'Clear Search History', self.on_clear_history, icon='edit-clear-symbolic') )
                ...
                menu = Gtk.Menu()
                menu.append( f_menu_item(1, 'Add Flag', self.on_edit_flag, args=(True,), icon='list-add-symbolic') )
                if self.selection_flag['id'] is not None:
                    menu.append( f_menu_item(1, 'Edit Flag', self.on_edit_flag, args=(False,), icon='edit-symbolic') )
                    menu.append( f_menu_item(1, 'Delete Flag', self.on_del_flag, icon='edit-delete-symbolic') )
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
                    menu.append( f_menu_item(1, 'Search for this Flag', self.add_tab, kargs={'type': FX_TAB_SEARCH, 'flag':self.selection_flag['id']}, icon='edit-find-symbolic'))  
                    menu.append( f_menu_item(1, 'Show Time Series for this Flag', self.add_tab,kargs={'type': FX_TAB_TIME_SERIES, 'flag':self.selection_flag['id']}, icon='office-calendar-symbolic'))  
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
                ...
                if self.selection_feed.get('id',0) > 0 and self.selection_feed['deleted'] != 1:
                    menu.append( f_menu_item(1, 'Show from newest...', self.add_tab, kargs={'type': FX_TAB_SEARCH, 'from_feed':True}, icon='edit-find-symbolic', tooltip="Show articles for this Channel or Category sorted from newest") )
                ...
                if self.selection_feed.get('id',0) > 0:
                    ...
                    if self.selection_feed['is_category'] == 1:
                        menu.append( f_menu_item(1, 'Move Category...', self.feed_win.copy_feed, icon='edit-cut-symbolic') ) 
                    else: menu.append( f_menu_item(1, 'Move Feed...', self.feed_win.copy_feed, icon='edit-cut-symbolic') ) 
                    ...
                    if self.feed_win.copy_selected.get('id') is not None and not self.flag_fetch and not self.flag_feed_update:
                        if self.feed_win.copy_selected['is_category'] == 1:
                            if self.selection_feed['is_category'] == 1:
                                menu.append( f_menu_item(1, f'Insert {esc_mu(self.feed_win.copy_selected.name())} here ...', self.feed_win.insert_feed, icon='edit-paste-symbolic') )
                        else:
                            if self.selection_feed['is_category'] == 1:
                                menu.append( f_menu_item(1, f'Assign {esc_mu(self.feed_win.copy_selected.name())} here ...', self.feed_win.insert_feed, icon='edit-paste-symbolic') ) 
                            else:
                                menu.append( f_menu_item(1, f'Insert {esc_mu(self.feed_win.copy_selected.name())} here ...', self.feed_win.insert_feed, icon='edit-paste-symbolic') )
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
                    if self.selection_feed['deleted'] == 1:
                        ...
                        if self.debug is not None: menu.append( f_menu_item(1, 'Technical details...', self.on_feed_details, icon='zoom-in-symbolic', tooltip="Show all technical information about this Channel") )
    ...
    def _on_right_click_prev(self, widget, menu, *args, **kargs):
        ...
        if self.selection_text != '':
            menu.append( f_menu_item(0, 'SEPARATOR', None) )
            menu.append( f_menu_item(1, 'Search for this Term', self.add_tab, kargs={'type': FX_TAB_SEARCH, 'phrase':self.selection_text}, icon='edit-find-symbolic'))  
            menu.append( f_menu_item(1, 'Show this Term\'s Contexts', self.add_tab, kargs={'type': FX_TAB_CONTEXTS, 'phrase':self.selection_text}, icon='view-list-symbolic'))  
            menu.append( f_menu_item(1, 'Show Terms related to this Term', self.add_tab, kargs={'type': FX_TAB_TERM_NET, 'phrase':self.selection_text}, icon='emblem-shared-symbolic'))  
            menu.append( f_menu_item(1, 'Show Time Series for this Term', self.add_tab,kargs={'type': FX_TAB_TIME_SERIES, 'phrase':self.selection_text}, icon='office-calendar-symbolic'))  
            menu.append( f_menu_item(0, 'SEPARATOR', None) )
            menu.append( f_menu_item(1, 'Search WWW', self._on_www_search_sel, icon='www-symbolic', tooltip="Search WWW for selected text" ) )
            menu.append( f_menu_item(0, 'SEPARATOR', None) )
            menu.append( f_menu_item(1, 'Add as Rule', self._on_add_as_rule, icon='view-list-compact-symbolic'))  
            menu.show_all()
    ...
    def add_tab(self, *args):
        ...
        elif type == FX_TAB_TREE:  self.upper_pages[tab_id].query('', kargs.get('filters',{'last':True, 'deleted':False, 'group': 'category', 'sort':'+importance'}))
        ...
        if phrase is not None and type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TERM_NET, FX_TAB_TIME_SERIES): 
            self.upper_pages[tab_id].query_entry.set_text(scast(phrase, str, ''))
        if qtype is not None and type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TERM_NET, FX_TAB_TIME_SERIES): 
            f_set_combo( self.upper_pages[tab_id].qtype_combo, qtype )
        if case_ins is not None and type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TERM_NET, FX_TAB_TIME_SERIES): 
            f_set_combo( self.upper_pages[tab_id].case_combo, case )
        if flag is not None and type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TERM_NET, FX_TAB_TIME_SERIES): 
            f_set_combo( self.upper_pages[tab_id].flag_combo, scast(flag, str, None) )


        # Add date range to filters if provided
        if date_range and type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TERM_NET, FX_TAB_TIME_SERIES):
        ...
        if type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TERM_NET, FX_TAB_TIME_SERIES): self.upper_pages[tab_id].query_entry.grab_focus()
    ...
    def _on_unb_changed(self, *args):
        ...
        if type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_SIMILAR, FX_TAB_TREE):
...
<a href="{esc_mu(l.replace('<','').replace('>',''))}" title="Click to open link">{esc_mu(l.replace('<','').replace('>',''))}</a>"""
...
# DIALOGS ADN ACTIONS FROM MENUS OR BUTTONS
    ...
    def on_clear_history(self, *args):
        ...
        if dialog.response == 1:
            ...
            for t in self.upper_pages:
                if t.type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_TERM_NET, FX_TAB_TIME_SERIES): t.reload_history()
        dialog.destroy()
    ...
    def on_del_flag(self, *args):
        ...
        dialog = YesNoDialog(self, 'Delete Flag', f'Are you sure you want to permanently delete <b><i>{esc_mu(flag.name())}</i></b> Flag?')           
        dialog.run()
        if dialog.response == 1:
            msg = flag.r_delete()
            self.update_status(0, msg) 
    ...
    def on_show_rules_for_entry(self, *args, **kargs):
        ...
        importance, flag, best_entries, flag_dist, rules = entry.matched_rules()
        footer = f"""Rules matched: <b>{len(rules)}</b>
Saved Importance: <b>{self.selection_res['importance']:.3f}</b>
Saved Flag: <b>{self.selection_res['flag']:.0f}</b>

Calculated Importance: <b>{importance:.3f}</b>
Caculated Flag: <b>{flag:.0f}</b>
...
Most similar read Entries:"""
        ...
        for r in rules: store.append( (r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8], r[9], r[10], r[11], r[12], r[13] ) )
...
#   DB Maintenance Stuff
    ...
    def on_maintenance(self, *args, **kargs):
        """ BD Maintenance """
        if self.flag_db_blocked: return -1
        dialog = YesNoDialog(self, 'DB Maintenance', f'Are you sure you want to DB maintenance? This may take a long time...')  
        dialog.run()
        if dialog.response == 1:
            self.action_q.append(FX_ACTION_BLOCK_DB)
            self.flag_db_blocked = True
    ...
    def _choose_file(self, *args, **kargs):
        ...
        if kargs.get('action') == 'save' and os.path.isfile(filename):
            dialog = YesNoDialog(self, f'Overwrite?', f'File <b>{esc_mu(filename)}</b> already exists!   Do you want to overwrite it?')
            dialog.run()
    ...
    def export_results_csv(self, *args):
        ...
        if self.upper_pages[self.curr_upper].type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_SIMILAR):
