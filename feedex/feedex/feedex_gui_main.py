...
class FeedexMainWin(Gtk.Window):
    ...
    def __init__(self, **kargs):
        ...
        self.FX = Feeder(config=self.config, debug=self.debug, ignore_images=self.config.get('ignore_images', False), gui=True, load_icons=True, main_thread=True)
        if self.FX.locked():
    ...
    def _on_timer(self, *kargs):
        ...
        for i,f in self.processing_flags.items():
            if self.upper_pages[i].processing_flag != f:
                if self.upper_pages[i].processing_flag == False:
                    self.upper_pages[i].finish_search()

                self.processing_flags[i] = not f
    ...
    def _apply_changes(self, id, action, **kargs):
        ...
        if id is None or action is None: return -1
    ...
    def update_status(self, *args):
        ...
        if spin != 3:
            self.status_bar.set_markup(parse_message(msg, debug=self.debug))
...
# DIALOGS ADN ACTIONS FROM MENUS OR BUTTONS
    ...
    def update_feed(self, *args):
        ...
        feed_id = args[-1]
        FX = Feeder(config=self.config, debug=self.debug, ignore_images=self.config.get('ignore_images',False), gui=True, timeout=1)
        for msg in FX.g_fetch(id=feed_id, update_only=True, force=True): self.message_q.append((1, msg,))
    ...
    def edit_entry(self, entry, *args):
        """ Add / Edit Entry """
        if entry is not None and self.curr_actions['edit_entry']: return 0

        if entry is None: dialog = EditEntry(self, self.config, self.entry_fields, new=True)
        else:
            if self.selection_res['id'] is None: return -1
            dialog = EditEntry(self, self.config, entry, new=False)
        dialog.run()
        if entry is None: self.entry_fields = dialog.result
        if dialog.response == 1:
            if entry is None: 
                self.update_status(1, 'Adding entry ...')
                t = threading.Thread(target=self.edit_entry_thr, args=(None, dialog.result) )
            else: 
                self.update_status(1, 'Updating entry ...')
                t = threading.Thread(target=self.edit_entry_thr, args=(self.selection_res['id'], dialog.result) )
            t.start()
        dialog.destroy()
    ...
    def edit_entry_thr(self, id, result, **kargs):
        ...
        if id is None:
            err = False
            for msg in FX.g_add_entries(elist=(result,), learn=result.get('learn',False)): 
                if msg[0] < 0: err = True
                self.message_q.append((1, msg,))
            if not err: 
                self.entry_fields = {}
            ...
            del result['learn']
            err = False
            for msg in FX.g_edit_entry(id, result):
                if msg[0] < 0: err = True
                self.message_q.append((1, msg))
    ...
    def on_del_entry(self, *args):
        ...
        if dialog.response == 1:
            err = False
            msg = self.FX.r_del_entry(self.selection_res['id'])
            if msg[0] < 0: err = True
    ...
    def on_restore_entry(self, *args):
        ...
        if dialog.response == 1:
            err = False
            for msg in self.FX.g_edit_entry(self.selection_res['id'], {'deleted': 0}): 
                if msg[0] < 0: err = True
                self.update_status(0, msg)
    ...
    def on_change_category(self, *args):
        ...
        dialog.run()
        if dialog.response == 1:
            msg = self.FX.r_edit_feed(self.selection_feed['id'], {'parent_id': dialog.result})
            self.update_status(0, msg)
            self.feed_win.reload_feeds(load=True)
        dialog.destroy()
    ...
    def edit_category(self, category):            
        """ Add/Edit Category """
        if category is None: dialog = EditCategory(self, self.new_category_fields, new=True)
        else: dialog = EditCategory(self, category, new=False)
        dialog.run()
        if category is None: self.new_category_fields = dialog.result
        if dialog.response == 1:
            err = False
            if category is None:
                msg = self.FX.r_add_feed(dialog.result.get('name'), {'subtitle':dialog.result.get('subtitle'), 'is_category':True})
                if msg[0]: err = True
                self.update_status(0, msg)
            else:
                msg = self.FX.r_edit_feed(category['id'], dialog.result, type='category')
                if msg[0] < 0: err = True
                self.update_status(0, msg)
    ...
    def edit_feed(self, feed):
        ...
        dialog.run()
        if feed is None: self.new_feed_fields = dialog.result
        if dialog.response == 1:
            if feed is None:
                name = dialog.result['name']
                del dialog.result['name']
                dialog.result['is_category'] = False
                msg = self.FX.r_add_feed(name, dialog.result)
                self.update_status(0, msg)
            else:
                msg = self.FX.r_edit_feed(feed['id'], dialog.result)
                self.update_status(0, msg)        
    ...
    def on_del_feed(self, *args):
        ...
        if self.selection_feed['is_category'] != 1:
            if self.selection_feed['deleted'] == 1:
                dialog = YesNoDialog(self, 'Delete Channel permanently', f'<b>Are you sure you want to permanently delete <i>{feed_name(self.selection_feed)}</i>?</b>')
            else:
                dialog = YesNoDialog(self, 'Delete Channel', f'<b>Are you sure you want to delete <i>{feed_name(self.selection_feed)}</i>?</b>')
            dialog.run()
            if dialog.response == 1:
                msg = self.FX.r_del_feed(self.selection_feed['id'], type='feed')
                self.update_status(0, msg)
                self.feed_win.reload_feeds(load=True)
            dialog.destroy()
            ...
            dialog.run()
            if dialog.response == 1:
                msg = self.FX.r_del_feed(self.selection_feed['id'], type='category')
                self.update_status(0, msg)
                self.feed_win.reload_feeds(load=True)
            dialog.destroy()
    ...
    def on_restore_feed(self, *args):
        ...
        if self.selection_feed['is_category'] == 1:
            ...
            dialog = YesNoDialog(self, 'Restore Channel', f'<b>Restore <i>{feed_name(self.selection_feed)}</i> Channel?</b>')
        dialog.run()
        if dialog.response == 1:
            msg = self.FX.r_edit_feed(self.selection_feed['id'], {'deleted': 0}) 
            self.update_status(0, msg)
            self.feed_win.reload_feeds(load=True)
        dialog.destroy()
    ...
    def on_mark_healthy(self, *args):
        ...
        msg = self.FX.r_edit_feed(self.selection_feed['id'], {'error': 0})
        self.update_status(0, msg)
        self.feed_win.reload_feeds(load=True)
    ...
    def mark_read(self, *args):
        ...
        for msg in FX.g_edit_entry(id, {'read': read+1}):
            if msg[0] < 0: err = True
            self.message_q.append((1, msg))
        self.message_q.append((3, None))
        self.curr_actions['edit_entry'] = False
    ...
    def on_mark_unread(self, *args):
        ...
        if self.selection_res['id'] is None: return 0        
        err = False
        for msg in self.FX.g_edit_entry(self.selection_res['id'], {'read': 0}):
            if msg[0] < 0: err = True
    ...
    def on_mark_flag(self, *args):
        ...
        flag = slist(args,-1,0)
        err = False
        for msg in self.FX.g_edit_entry(self.selection_res['id'], {'flag': flag}):
            if msg[0] < 0: err = True
            self.update_status(1, msg)
        self.update_status(3, None)
        if err: return -1
    ...
    def on_mark_unflag(self, *args):
        ...
        if self.selection_res['id'] is None: return 0        
        err = False
        for msg in self.FX.g_edit_entry(self.selection_res['id'], {'flag': 0}):
            if msg[0] < 0: err = True
            self.update_status(1, msg)
        self.update_status(3, None)
        if err: return -1
    ...
    def on_del_rule(self, *args):
        ...
        dialog.run()
        if dialog.response == 1:
            msg = self.FX.r_del_rule(self.selection_rule['id']) 
            self.update_status(0, msg) 
            self._reload_rules()
        dialog.destroy()
    ...
    def on_del_query_rules(self, *args):
        ...
        dialog.run()
        if dialog.response == 1:
            msg = self.FX.r_delete_query_rules()
            self.update_status(0, msg) 
            self._reload_rules()
        dialog.destroy()
    ...
    def edit_rule(self, rule):
        ...
        dialog.run()
        if rule is None: self.new_rule_fields = dialog.result
        if dialog.response == 1:
            err = False
            if rule is None:
                msg = self.FX.r_add_rule(dialog.result) 
                if msg[0] < 0: err = True
                self.update_status(0, msg)
                if not err: self.new_rule_fields = {}
            else:
                msg = self.FX.r_edit_rule(rule['id'], dialog.result) 
                if msg[0] < 0: err = True
                self.update_status(0, msg)
    ...
    def on_view_log(self, *args):
        ...
        try:
            ...
            sys.stderr.write(f'Could not open log file ({self.config.get("log","<< UNKNOWN >>>")}): {e}')
            return -1
