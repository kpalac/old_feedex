...
# DIALOGS ADN ACTIONS FROM MENUS OR BUTTONS
    ...
    def on_del_feed(self, *args):
        ...
        if coalesce(feed['is_category'],0) == 0 and coalesce(feed['deleted'],0) == 0:
            ...
            dialog = YesNoDialog(self, 'Delete Channel permanently', f'<b>Are you sure you want to permanently delete <i>{esc_mu(feed.name_())}</i>?</b>')
    ...
    def mark_read_thr(self, *args):
        ...
        err = False
        for msg in entry.g_update({'read': scast(entry['read'],int,0)+1}):
            if msg[0] < 0: err = True
            self.lock.acquire()
            self.message_q.append((1, msg))
            self.lock.release()
    ...
    def on_mark_read(self, *args):
        ...
        self.curr_actions['edit_entry'] = True
        t = threading.Thread(target=self.mark_read_thr)
        t.start()
    ...
    def show_learned_rules(self, *args):
        ...
        for r in self.FX.MC.rules:
            if r[rule.get_index('learned')] == 1:
                ...
                rule_store.append( (r[rule.get_index('id')], r[rule.get_index('name')], r[rule.get_index('string')], r[rule.get_index('weight')], qtype) )
