...
It will also take some time to perform"""))  
        ...
        self.search_menu.append( f_menu_item(0, 'SEPARATOR', None) )
        self.search_menu.append( f_menu_item(1, 'Rules', self.add_tab, kargs={'type':FX_TAB_RULES}, icon='view-list-compact-symbolic', tooltip='Open a new tab showing Saved Rules') )
        self.search_menu.append( f_menu_item(1, 'Flags', self.add_tab, kargs={'type':FX_TAB_FLAGS}, icon='marker-symbolic', tooltip='Open a new tab showing Flags') )
    ...
    def _rbutton_press(self, widget, event, from_where):
        ...
        if event.button == 3:
            ...
            if from_where in (FX_TAB_RESULTS, FX_TAB_CONTEXTS):
                menu = Gtk.Menu()
                
                if not (self.curr_upper == 0 and self.curr_place == FX_PLACE_TRASH_BIN):
                ...
                if self.upper_pages[self.curr_upper].type in (FX_TAB_SEARCH, FX_TAB_CONTEXTS):
                    menu.append( f_menu_item(0, 'SEPARATOR', None) )
    ...
    def _on_unb_changed(self, *args):
        ...
        if type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_SIMILAR):
