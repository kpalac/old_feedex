...
class FeedexMainWin(Gtk.Window):
    ...
    def __init__(self, feedex_main_container, **kargs):
        ...
        self.search_menu.append( f_menu_item(0, 'SEPARATOR', None) )
        self.search_menu.append( f_menu_item(1, 'Rules', self.add_tab, kargs={'type':FX_TAB_RULES}, icon='view-list-compact-symbolic', tooltip='Open a new tap showing Saved Rules') )
        ...
        if event.button == 3:
            ...
            if from_where in (FX_TAB_RESULTS, FX_TAB_CONTEXTS):
                ...
                if self.selection_res['id'] is not None:
                    ...
                    author_menu.append( f_menu_item(1, 'Activity in Time', self.add_tab, kargs={'type': FX_TAB_TIME_SERIES, 'author': True}, icon='office-calendar-symbolic', tooltip="Search for other documents by this Author" ) )
                    author_menu.append( f_menu_item(0, 'SEPARATOR', None) )
                    author_menu.append( f_menu_item(1, 'Search WWW', self._on_www_search_auth, icon='network-workgroup-symbolic', tooltip="Search WWW for this Author's info" ) )
    ...
    def _on_right_click_prev(self, widget, menu, *args, **kargs):
        ...
        if self.selection_text != '':
            ...
            menu.append( f_menu_item(0, 'SEPARATOR', None) )
            menu.append( f_menu_item(1, 'Search WWW', self._on_www_search_sel, icon='network-workgroup-symbolic', tooltip="Search WWW for selected text" ) )
            menu.append( f_menu_item(0, 'SEPARATOR', None) )
    ...
    def _on_timer(self, *kargs):
        ...
        if len(self.action_q) > 0:
            ...
            else: self.upper_pages[self.curr_upper].apply_changes(uid, action)
