...
class LingProcessor:
    ...
    def set_model(self, model:str, **kargs):
        ...
        if (model == None or model == 'heuristic') and kargs.get('detect',True) and self.fields not in ([],()):
    ...
    def _extract_patterns(self, field, **kargs):
        ...
        meta = kargs.get('meta', slist(PREFIXES.get(field), 4, False))
        field_feat = kargs.get('field_feat', slist(PREFIXES.get(field), 7, []))
    ...
    def process(self, string:str, field:int, **kargs):
        ...
        stem = kargs.get('stem', slist(PREFIXES.get(field,[]), 3, False)) # Should field's tokens be stemmed?
        meta = kargs.get('meta', slist(PREFIXES.get(field,[]), 4, False)) # ... or maybe they are meta information?
        ignore = kargs.get('ignore', slist(PREFIXES.get(field,[]), 6, [])) 
        field_feat = kargs.get('field_feat', slist(PREFIXES.get(field,[]), 8, [])) # ... or every token in this field contains valuable data?
