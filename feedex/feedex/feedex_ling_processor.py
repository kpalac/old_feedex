            ...
            if os.path.isdir(FEEDEX_MODELS_PATH):
                for header_file in os.listdir(FEEDEX_MODELS_PATH):
                    ...
                    if header_file.endswith('_header.pkl') and os.path.isfile(header_file):
                        ...
                        if self.debug in (1,6,4): print(f'Pickle {header_file} loaded...')
    ...
    def _load_pickle(self, pkl:str, **kargs):
        ...
        if os.path.isfile(pickle_path):
            try:
                ...
                lst = {}
            if self.debug in (1,6,4): print(f'Pickle {pkl} loaded...')
        else:
    ...
    def match_rules(self, **kargs):
        ...
        for r in self.MC.rules:
            ...
            if matched > 0:
                ...
                if to_var: 
                    if learned == 1: disp_learned = 'YES'
                    else: disp_learned = 'NO'

                    if case_ins: disp_case_ins = 'YES'
                    else: disp_case_ins = 'NO'

                    if additive: disp_additive = 'YES'
                    else: disp_additive = 'NO'

                    if feed == -1: disp_feed = 'ALL'
                    else: disp_feed = str(feed)

                    if qtype == 0: disp_type = 'String Matching'
                    elif qtype == 1: disp_type = 'Full Text Search'
                    elif qtype == 2: disp_type = 'Full Text Search (Exact)'
                    elif qtype == 3: disp_type = 'REGEX'
                    elif qtype == 4: disp_type = 'Learned rule text search'
                    elif qtype == 5: disp_type = 'Learned rule text search (Exact)'
                    else: disp_type = '<UNKNOWN>'

                    if do_flag > 0: disp_flag_name = self.MC.flags.get(do_flag, ('<NONE>',) )[0]
                    else: disp_flag_name = '<NONE>' 

                    display_list.append( (name, string, matched, disp_learned, disp_case_ins, disp_type, field, disp_feed, lang, weight, do_flag, disp_flag_name, disp_additive, context_id) )
