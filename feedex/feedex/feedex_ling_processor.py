...
class LingProcessor:
    ...
    def __init__(self, **kargs):
        ...
        if type(self.multi_dict) is not dict: self.multi_dict = {}
    ...
    def set_model(self, model:str, **kargs):
        ...
        if (model is None or model == 'heuristic') and kargs.get('detect',True) and self.fields not in ([],()):
        ...
        if self.model == {}:
            if kargs.get('detect',True) and self.fields != []:
                self.model = self.models.get(self.detect_lang(), self.models.get('heuristic',{}))
            else:
        ...
        if self.model.get('writing_system',0) != 0:
            if self.model.get('stemmer') is not None:
        ...
        if mname not in self.loaded:
            ...
            for d,v in tmp_d.items(): self.models[mname]['d'][d] = v
    ...
    def _extract_patterns(self, field, **kargs):
        ...
        field_weight = PREFIXES[field][5]
        meta = kargs.get('meta', PREFIXES[field][4])
        field_feat = kargs.get('field_feat', PREFIXES[field][7])

        if field_feat:
            prefix = PREFIXES[field][0]
            field_name = f"""{PREFIXES[field][2]}: """
        else:
            prefix = '..'
            field_name = ''
    ...
    def _prefix(self, field=-1, **kargs):
        """ Ascertain a prefix for a token based on field and case """
        if field in (None,-1):
            prefix = f"""T{kargs.get('wildcard','.')}"""
        else:
    ...
    def process(self, string:str, field:int, **kargs):
        """ Process a single field """

        self.token_str = f'{self.token_str}   '

        stem = kargs.get('stem', PREFIXES[field][3]) # Should field's tokens be stemmed?
        meta = kargs.get('meta', PREFIXES[field][4]) # ... or maybe they are meta information?
        ignore = kargs.get('ignore', PREFIXES[field][6]) # Strings to ignore
        field_feat = kargs.get('field_feat', PREFIXES[field][7]) # ... or every token in this field contains valuable data?
    ...
    def process_fields(self, fields:list, **kargs):
        ...
        if stats:
            ...
            if self.stats.get('readability') is not None:
    ...
    def detect_lang(self):
        ...
        for t in self.fields:
            if PREFIXES.get("i",[None,None,None,None])[3] is False:
                continue                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
            ...
            if not self.models[l].get('logographic',False):
                ...
                for c in chars:
                    ...
                    if c in self.models[l].get('unique_chars',''):
                        langs[l] = v + 10  #This is a big deal, so it is weighted quite highly
