...
class LingProcessor:
    ...
    def match_rules(self, **kargs):
        ...
        contexts_sorted = sorted(entry_dist.items(), key=lambda x:x[1], reverse=True)
...
"""
    ...
    def f_phrase(self, string:str, **kargs):
        ...
        def _repl_wc_sql():
            phrase['sql'] = phrase['sql'].replace('\*',rstr)
            phrase['sql'] = phrase['sql'].replace('\.',rstr2)
            phrase['sql'] = phrase['sql'].replace('*','%')
            phrase['sql'] = phrase['sql'].replace('.','_')
            phrase['sql'] = phrase['sql'].replace(rstr, '*')
            phrase['sql'] = phrase['sql'].replace(rstr2, '.')
