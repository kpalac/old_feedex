...
class FeedexRSSHandler:  
    ...
    def __init__(self, FX, **kargs):
        ...
        self.debug = kargs.get('debug', self.FX.config)
    ...
    def download(self, url:str, **kargs):
        ...
        try:
            ...
            if self.debug: sys.stderr.write(f'{e}\n')
            self.error = True
            return -1, f'{e}'
        ...
        if self.status == 304:
            ...
            if not do_redirects: return f'Feed {url} moved to new location!'
                ...
                if new_url not in (None,''):
                    ...
                    msg = self.download(new_url, etag=etag, modifed=modified, login=login, password=password, auth=auth, agent=self.agent, do_redirects=False, force=force, redirected=True)
                    return msg
                else:
            ...
            if self.debug: print(f"""{scast(self.status, str, '')} {scast(feed_raw.get('debug_message'), str, ' Feed error')}""")
            self.error = True
            return f'Invalid HTTP return code for {url} ({self.statusstatus})'
        ...
        for i in self.images:
            ...
            if href != '' and feed_id != 0:
                ...
                try:
                    ...
                    return f'Could not download image from {href}!'
            else:
                continue
