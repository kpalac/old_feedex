...
class FeedexRSSHandler:  
    ...
    def __init__(self, FX, **kargs):
        ...
        self.debug = kargs.get('debug', self.FX.config.get('debug',False))
        ...
        if self.status == 304:
            ...
            if not do_redirects: return f'Feed {url} moved to new location!'
                ...
                if new_url not in (None,''):
                    ...
                    msg = self.download(new_url, etag=etag, modifed=modified, login=login, password=password, auth=auth, do_redirects=False, force=force, redirected=True)
                    return msg
                else:
