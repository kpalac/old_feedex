...
class FeedexRSSHandler:  
    ...
    def download(self, url:str, **kargs):
        ...
        agent = kargs.get('agent',FEEDEX_USER_AGENT)
        ...
        try:
            ...
            if self.debug: sys.stderr.write(e)
            self.error = True
            return -1, e
        ...
        if status in (0,304):
            ...
            if not do_redirects:
                self.changed = False
                return f'Feed {url} prermanently moved to new location!'
            else:
            ...
            self.error = True
            return f'Invalid HTTP return code for {url} ({status})'
    ...
    def _get_images(self):
        ...
        for i in self.images:
            ...
            if type(href) is str and feed_id != 0:
    ...
    def update(self, feed, **kargs):
        ...
        if self.feed_raw.get('feed',None) is None: return f'Downloaded feed emtpty ({feed_name_cli(feed)})!'
