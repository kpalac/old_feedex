...
class RuleContainer(RuleContainerBasic):
    ...
    def validate(self, **kargs):
        ...
        if self.vals.get('case_insensitive') is not None:
            if self.vals.get('case_insensitive') not in (0,1): return -7, 'Case insensitivity must be 0 or 1!' 
...
class FlagContainer(FlagContainerBasic):
    ...
    def validate(self, **kargs):
        ...
        if self.vals['id'] in self.FX.MC.flags.keys() and self.vals['id'] != self.backup_vals['id']: return -7, 'ID taken by another flag'
        if self.vals['name'] is None or self.vals['name'] == '': return -7, 'Flag name cannot be empty!'
        if self.vals['color_cli'] is not None and self.vals['color_cli'] not in TCOLS.keys(): return -7, 'Invalid CLI color name!'
    ...
    def r_do_update(self, **kargs):
        ...
        if self.FX.rowcount > 0:
            ...
            if not self.FX.single_run: 
                ...
                if err != 0: return -2, f'Error reloading flags after successfull update: %a', err
            ...
            if len(self.to_update) > 1:
                ...
                return 0, f'Flag %a updated successfuly!', self.name()
            else:
                for f in self.to_update:
                    if f == 'id':
                        self.FX.log(False, f'Flag {self.name()}: ID changed from {self.backup_vals.get("id","<UNKNOWN>")} to {self.vals.get("id","<UNKNOWN>")}')
                        return 0, f'Flag %a: ID changed from {self.backup_vals.get("id","<UNKNOWN>")} to {self.vals.get("id","<UNKNOWN>")}', self.name()
                    else:
                        self.FX.log(False, f'Flag {self.name()} updated:  {f} -> {self.vals.get(f,"<NULL>")}')
                        return 0, f'Flag %a updated:  {f} -> {self.vals.get(f,"<NULL>")}', self.name()
    ...
    def r_delete(self, **kargs):
        ...
        if self.FX.rowcount > 0: 
            if not self.FX.single_run: 
                err = self.FX.load_flags()
                if err != 0: return -2, f'Error reloading flags after successfull delete: %a', err

            self.FX.log(False, f'Flag {self.vals["id"]} deleted')
            return 0, 'Flag %a deleted', self.vals['id']
    ...
    def r_add(self, **kargs):
        ...
        if not self.FX.single_run: 
            err = self.FX.load_flags()
            if err != 0: return -2, f'Error reloading flags after successfull add: %a', err

        self.FX.log(False, f'Flag {self.name(id=True)} added')
        return 0, f'Flag %a added successfully', self.name(id=True)
