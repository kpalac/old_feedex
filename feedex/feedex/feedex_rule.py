...
class RuleContainer(RuleContainerBasic):
    ...
    def validate(self, **kargs):
        ...
        if self.vals['string'] is None or self.vals['string'] == '': return -1, 'Search string cannot be empty!'

        if self.vals['flag'] not in (None, 0,1,2,3,4,5): return -1, 'Flag needs to be empty or in (1,2,3,4,5)'
    
        if self.vals.get('qtype') is not None:
            qtype = self.FX.resolve_qtype(self.vals['qtype'])
            if qtype == -1: return -1, 'Invalid query type! Must be string(0), full-text (1), exact (2) or regex (3)'
            else: self.vals['type'] = qtype

        if self.vals.get('type') not in (0,1,2,3): return -1, 'Invalid query type! Must be string(0), full-text (1), exact (2) or regex (3)'

        
        if 'feed' in self.vals.keys():
            if self.vals.get('feed') is not None:
                feed = self.FX.resolve_feed(self.vals['feed'], load=True)
                if feed == -1: return -1, 'Channel %a not found!', self.vals.get('feed', '<UNKNOWN>')
                else: 
            ...
            if self.vals.get('feed_or_cat') is not None:
                ...
                if feed == -1: return -1, 'Channel/Category %a not found!', self.vals.get('feed', '<UNKNOWN>')
                else: 
                    self.feed.populate(feed)
                    self.vals['feed_id'] = self.feed['id']
            else: self.vals['feed_id'] = None
        ...
        if 'field' in self.vals.keys():
            if self.vals.get('field') is not None:
                ...
                if field == -1: return -1, 'Field to search not valid!'
        ...
        if self.vals['type'] == 3 and not check_if_regex(self.vals['string']): return -1, 'Not a valid regex string!'
        ...
        if self.vals['case_insensitive'] not in (0,1): return -1, 'Case insensitivity must be 0 or 1!'
    ...
    def r_do_update(self, **kargs):
        ...
        if not self.exists: return 0, 'Rule not found'
...
class HistoryItem(SQLContainerEditable):
    ...
    def validate(self, **kargs):
        ...
        if self.vals['feed_id'] is not None:
            ...
            if feed_id == -1: return -1, 'Channel/Category %a not found!', self.vals.get('feed_id')
        return 0


    def add(self, phrase:dict, string:str, feed:int, **kargs):
        """ Wrapper for adding item to search history """
        self.vals['id'] = None

        string = nullif(scast(string, str, '').strip().replace('*',''), '')

        if not phrase.get('emtpy',False) and (string is not None):
        ...
        err = self.FX.run_sql_lock(self.insert_sql(all=True), self.vals)
        if err != 0: return -1 , 'DB error: %a', err
